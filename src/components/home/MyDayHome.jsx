...
export default function MyDayHome({ dispatcher }) {
  ...
  useEffect(() => {
    let dead = false
    const load = () => {
      getJSON('/api/home/me').then(d => { if (!dead) { setMe(d); setErr('') } }).catch(e => { if (!dead) setErr(e.message) })
      if (dispatcher) getJSON('/api/dispatch/center?day=0').then(d => { if (!dead) setCenter(d) }).catch(() => {})
    }
    load()
    const t = setInterval(load, dispatcher ? 90_000 : 120_000)
        ...
        <div style={{ padding: isMobile ? '12px 12px 24px' : '20px 26px 96px', display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 1480 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'minmax(0, 1fr)' : 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 16, alignItems: 'start' }}>
            <Section title="Next up" sub={cards.length > 4 ? `${cards.length - 4} more in the Command Center` : null} link={['Command Center', '/dispatch']} onGo={go}>
              {!center ? <div className="skel" style={{ height: 120, borderRadius: 12 }} /> : cards.length ? cards.slice(0, 4).map((c, i) => {
              ...
              }) : <Empty>Nothing needs a decision right now. New bookings and late techs show up here as they happen.</Empty>}
            </Section>
  ...
  const kpis = [
    ...
    { label: 'Calls answered', value: String(s.handled ?? 0), sub: s.avgTalkSec != null ? `avg talk ${mmss(s.avgTalkSec)}` : 'in Andi today' },
