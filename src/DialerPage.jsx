...
export default function DialerPage() {
  ...
  const playRecording = (id, url) => {
    ...
    if (url?.includes('api.twilio.com')) {
      const sid = url.split('/').pop()?.replace('.mp3', '')
      src = `/api/twilio/recording/${sid}`
    } else {
      // ServiceTitan / other CDN URLs can't be loaded directly in the
      // browser (auth + CORS), so proxy them through the backend.
      src = `/api/st/recording?url=${encodeURIComponent(url)}`
    }
