...
export default function DashboardPage() {
  ...
  // All contacts in timeframe based on their last update
  const { start: tfStart, end: tfEnd } = getTimeframeBounds(tf)

  const total = contacts.length
  const dupSet = getDupSet(contacts)

  // Timeframe-filtered contacts (booked/done within the selected period)
  const tfContacts = tf === 'all'
    ? contacts
    : contacts.filter(c => {
        const d = new Date(c.updated_at || c.created_at)
        return d >= tfStart && d <= tfEnd
      })

  const tfBooked = logs.filter(l => l.outcome === 'Booked').length
  const tfDone = logs.filter(l => isDone({ status: l.outcome })).length
  const tfCalls = logs.length
  const tfConv = tfCalls ? Math.round((tfBooked / tfCalls) * 100) : 0

  // All-time pipeline stats
  const allBooked = contacts.filter(c => c.status === 'Booked').length
  const allDone = contacts.filter(isDone).length
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        {TF_OPTIONS.map(t => (
          <button key={t} onClick={() => setTf(t)}
            style={{
              padding: '4px 10px', borderRadius: 99, fontSize: 11, fontWeight: 500, border: '1px solid', cursor: 'pointer',
      ...
      {/* Call activity — timeframe filtered */}
      <div>
      ...
      {/* Rep performance — timeframe filtered */}
      {!logsLoading && (
        <div className="card">
              ...
              return (
                <div key={rep} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '14px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    ...
                    <Badge status="Booked">{d.booked} booked</Badge>
                  </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10 }}>
          {[
            ...
            { label: 'Remaining', value: (total - allDone).toLocaleString(), color: 'warning', onClick: () => setDrilldown({ title: 'Active contacts', rows: contacts.filter(c => !isDone(c)) }) },
            { label: 'Total booked', value: allBooked.toLocaleString(), sub: `${total ? Math.round((allBooked / total) * 100) : 0}% conversion`, color: 'success', onClick: () => setDrilldown({ title: 'Booked contacts', rows: contacts.filter(c => c.status === 'Booked') }) },
            { label: 'Completed', value: allDone.toLocaleString(), sub: `${pct}% of list`, color: 'purple' },
