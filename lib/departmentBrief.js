...
// ServiceTitan's Sales close (lib/salesClose.js — the Technician Scorecard /
// BU dashboard number) for a window, bucketed by the JOB's business unit.
// Department briefs are per-BU, so no per-tech pass is needed.
const blankClose = () => ({ opps: 0, closed: 0, rate: null })
async function salesCloseByTrade({ stGet, stPageAll, tenantId, startIso, endIso }) {
  const bus = await stGet(`/settings/v2/tenant/${tenantId}/business-units?pageSize=200`).then(d => d?.data || []).catch(() => [])
  const buName = new Map(bus.map(b => [String(b.id), b.name || '']))
  const result = computeSalesClose(await fetchSalesCloseInputs({ stGet, stPageAll, tenantId, startIso, endIso, primary: false }))
  const bucket = (job) => tradeOfBU(buName.get(String(job?.businessUnitId)))
  const byTrade = rollupSalesClose(result, { groupOfTech: (tech, job) => bucket(job), groupOfJob: bucket })
  const forTrade = (trade) => {
    if (trade !== 'ALL') return byTrade[trade] || blankClose()
    const all = blankClose()
    for (const t of ALL_TRADES) { const c = byTrade[t]; if (c) { all.opps += c.opps; all.closed += c.closed } }
    all.rate = all.opps ? all.closed / all.opps : null
    return all
  }
  return { result, byTrade, forTrade }
}
...
export async function gatherDepartmentFacts({ stGet, stPageAll, supabase, tenantId, trade, dateStr }) {
  ...
  // ── Close rate: ServiceTitan's Sales close for this trade ─────────────────
  const closeTrade = await salesCloseByTrade({ stGet, stPageAll, tenantId, startIso, endIso })
    .then(c => c.forTrade(trade)).catch(e => { console.warn('deptbrief close:', e.message); return blankClose() })
  const closeRate = closeTrade.rate
  ...
  return {
    ...
    opportunities: closeTrade.opps, sold: closeTrade.closed,
...
// ── Daily metrics series: sales, calls, sales-per-call, by day ──────────────
// Answers "X per day, sorted" questions. Call-based metrics need telecom,
// which is heavy over long windows, so they're included up to 31 days; beyond
// that, sales/revenue per day still return but call metrics are null.
export async function buildDailyMetrics({ stPageAll, supabase, tenantId, trade, days }) {
  const dc = Math.min(Math.max(Number(days) || 7, 1), 90)
  const w = periodWindow(String(dc))
  const withCalls = dc <= 31
  const denverDay = (iso) => { const d = new Date(String(iso).replace('Z', '+00:00')); d.setUTCHours(d.getUTCHours() - 6); return d.toISOString().slice(0, 10) }
  const safe = (p, f) => p.then(r => r).catch(() => f)
  const [sold, inv, rawCalls, admins] = await Promise.all([
    safe(stPageAll(pg => `/sales/v2/tenant/${tenantId}/estimates?soldAfter=${w.startIso}&soldBefore=${w.endIso}&pageSize=500&page=${pg}`, 9000), []),
    safe(stPageAll(pg => `/accounting/v2/tenant/${tenantId}/invoices?invoicedOnOrAfter=${w.fromDate}T00:00:00Z&invoicedOnBefore=${w.toDate}T23:59:59Z&pageSize=500&page=${pg}`, 9000), []),
    withCalls ? safe(stPageAll(pg => `/telecom/v2/tenant/${tenantId}/calls?createdOnOrAfter=${w.startIso}&createdBefore=${w.endIso}&pageSize=500&page=${pg}`, 12000), []) : Promise.resolve([]),
    supabase ? loadAdminAgents(supabase) : Promise.resolve(null),
  ])
  const calls = admins ? rawCalls.filter(c => !admins.isAdminCall(c)) : rawCalls   // admins' test calls don't count
  const day = {}
  const D = (k) => (day[k] = day[k] || { date: k, sales: 0, soldCount: 0, revenue: 0, leadCalls: 0, booked: 0 })
  for (const e of sold) if ((e.status || {}).name === 'Sold' && tradeMatches(trade, e.businessUnitName) && e.soldOn) { const r = D(denverDay(e.soldOn)); r.sales += Number(e.subtotal) || 0; r.soldCount++ }
  for (const i of inv) if (tradeMatches(trade, (i.businessUnit || {}).name) && i.invoiceDate) { D(i.invoiceDate.slice(0, 10)).revenue += Number(i.subTotal) || 0 }
  if (withCalls) for (const c of calls) {
    const lc = c.leadCall || c
    if ((lc.direction || '') !== 'Inbound' || !['Booked', 'Unbooked'].includes(lc.callType)) continue
    if (trade !== 'ALL' && !tradeMatches(trade, (lc.campaign || {}).businessUnit)) continue
    const r = D(denverDay(lc.receivedOn || lc.createdOn)); r.leadCalls++; if (lc.callType === 'Booked') r.booked++
  }
  const series = Object.values(day).sort((a, b) => a.date.localeCompare(b.date)).map(r => ({
    date: r.date, dayOfWeek: new Date(`${r.date}T12:00:00Z`).toLocaleDateString('en-US', { weekday: 'short', timeZone: 'UTC' }),
    sales: r0(r.sales), soldCount: r.soldCount, revenue: r0(r.revenue),
    leadCalls: withCalls ? r.leadCalls : null, booked: withCalls ? r.booked : null,
    salesPerCall: withCalls && r.leadCalls ? r0(r.sales / r.leadCalls) : null,
    revenuePerCall: withCalls && r.leadCalls ? r0(r.revenue / r.leadCalls) : null,
    bookingRate: withCalls && r.leadCalls ? Math.round(r.booked / r.leadCalls * 100) / 100 : null,
  }))
  return {
    scope: trade === 'ALL' ? 'company' : trade, window: `${w.fromDate} to ${w.toDate}`, days: dc,
    note: withCalls ? 'salesPerCall = sales dollars / inbound lead calls (Booked+Unbooked) that day.'
                    : 'Window > 31 days: per-call metrics omitted for performance; sales and revenue per day are included.',
    series,
  }
}
...
export async function buildCompanyOverview({ stGet, stPageAll, supabase, tenantId, anthropicKey, period }) {
  ...
  const closeBy = await salesCloseByTrade({ stGet, stPageAll, tenantId, startIso: w.startIso, endIso: w.endIso })
    .catch(e => { console.warn('company close:', e.message); return null })
  const departments = ALL_TRADES.map(t => {
    const d = byTrade[t]
    const c = closeBy ? closeBy.forTrade(t) : blankClose()
    return { department: t, sales: r0(d.sales), soldCount: d.soldCount, revenue: r0(d.revenue),
             ...
             opportunities: c.opps, soldJobs: c.closed, closeRate: c.rate == null ? null : Math.round(c.rate * 100) / 100 }
  })
...
export async function buildTechnicianPerformance({ stGet, stPageAll, supabase, tenantId, trade, days }) {
  ...
  // Close rate per tech = their line on ST's Technician Scorecard (lib/salesClose.js).
  const closeRes = await fetchSalesCloseInputs({ stGet, stPageAll, tenantId, startIso: w.startIso, endIso: w.endIso })
    .then(computeSalesClose).catch(e => { console.warn('techperf close:', e.message); return null })
  const techs = []
  for (const [id, jobs] of techJobs) {
    const tradeJobs = [...jobs].filter(j => jobTrade.get(j))
    if (!tradeJobs.length) continue
    const st = closeRes ? techStats(closeRes, id) : blankClose()
    const p = st.opps
    const s = st.closed
  ...
  return {
    ...
    note: "Close rate = ServiceTitan's Sales close (the Technician Scorecard number): opportunities = completed chargeable sales jobs where the tech held the first appointment, plus jobs they closed; closed = the job's sold estimates first reached the $90 sold threshold on their sale. Techs with few opportunities have noisy rates.",
    technicians: techs,
  }
}
...
export async function buildPeriodSummary({ stGet, stPageAll, supabase, tenantId, anthropicKey, trade, period }) {
  ...
  const salesTotal = sold.reduce((a, e) => a + (Number(e.subtotal) || 0), 0)
  const soldJobIds = new Set(sold.map(e => e.jobId).filter(Boolean))
  const closeTrade = await salesCloseByTrade({ stGet, stPageAll, tenantId, startIso: w.startIso, endIso: w.endIso })
    .then(c => c.forTrade(trade)).catch(e => { console.warn('summary close:', e.message); return blankClose() })
  const revTrade = invoices.filter(i => tradeMatches(trade, (i.businessUnit || {}).name))
  const revenueTotal = revTrade.reduce((a, i) => a + (Number(i.subTotal) || 0), 0)
  ...
  const facts = {
    ...
    revenue: { total: r0(revenueTotal), invoices: revTrade.length },
    closeRate: closeTrade.rate == null ? null : Math.round(closeTrade.rate * 100) / 100,
    opportunities: closeTrade.opps, soldJobs: closeTrade.closed,
...
export async function buildLeadSources({ stPageAll, supabase, tenantId, trade, days }) {
  const w = periodWindow(String(Math.min(Math.max(Number(days) || 14, 1), 90)))
  const [rawCalls, admins] = await Promise.all([
    stPageAll(pg => `/telecom/v2/tenant/${tenantId}/calls?createdOnOrAfter=${w.startIso}&createdBefore=${w.endIso}&pageSize=500&page=${pg}`, 10000).catch(() => []),
    supabase ? loadAdminAgents(supabase) : Promise.resolve(null),
  ])
  const calls = admins ? rawCalls.filter(c => !admins.isAdminCall(c)) : rawCalls   // admins' test calls don't count
  const chan = new Map()
