...
export function createFieldPro({ supabase, stGet, tenantId, sendResend, techMeta, anthropicKey, orgToken, clientId, clientSecret }) {
  ...
  async function emailConfig() {
    ...
    return { cfg: { enabled: false, minScore: 3, to: [], ...cfg }, mgrs }
  }
  async function recipientsFor(f, cfg, mgrs) {
    const users = await siroUsers()
    const tech = users.get(f.siro_user_id)
    const mgr = f.trade ? mgrs[f.trade] : null
    const list = [...(cfg.to || []), mgr?.email, tech?.email].filter(Boolean).map(s => String(s).trim().toLowerCase())
    return { to: [...new Set(list)], techEmail: tech?.email || null, manager: mgr || null }
  }

  const TYPE_LABEL = { REHASH: 'Rehash — unsold quote', CROSS_SELL: 'Cross-sell opportunity' }
  function reengageEmail(f, rec, { manager } = {}) {
    const c = f.context || {}
    const C = { ink: '#0F172A', mute: '#64748B', line: '#E2E8F0', soft: '#F8FAFC', accent: '#ff751f', good: '#15803D', warn: '#B45309' }
    const first = String(f.tech_name || 'the tech').split(/\s+/)[0]
    const cost = Number(String(c.overall_cost || '').replace(/[^0-9.]/g, ''))
    const who = f.customer_name || 'this customer'
    ...
    const chip = (text, bg, fg) => `<span style="display:inline-block;font-size:11.5px;font-weight:700;border-radius:99px;padding:3px 10px;margin:0 6px 6px 0;background:${bg};color:${fg}">${esc(text)}</span>`
    const block = (title, body, opts = {}) => body ? `
      <div style="margin-top:18px">
        <div style="font-size:10.5px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:${C.mute};margin-bottom:6px">${esc(title)}</div>
        <div style="font-size:14px;line-height:1.55;color:${C.ink};white-space:pre-wrap;${opts.box ? `background:${C.soft};border:1px solid ${C.line};border-radius:10px;padding:12px 14px;` : ''}">${esc(body)}</div>
      </div>` : ''
    const objections = [c.main_objection, c.secondary_objection].filter(x => x && !/^none$/i.test(String(x).trim())).join(' · also ')
    const stUrl = f.st_job_id ? `https://go.servicetitan.com/#/Job/Index/${f.st_job_id}` : null
    const btn = (href, label, primary) => href ? `<a href="${esc(href)}" style="display:inline-block;margin:0 8px 8px 0;padding:10px 16px;border-radius:99px;font-size:13px;font-weight:700;text-decoration:none;${primary ? `background:${C.accent};color:#fff` : `border:1px solid ${C.line};color:${C.ink}`}">${esc(label)}</a>` : ''
    const html = `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:640px;margin:0 auto;color:${C.ink};padding:8px">
      <div style="font-size:11px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:${C.accent}">Re-Engage · Field Pro</div>
      <div style="font-size:21px;font-weight:800;margin:4px 0 2px">Follow up with ${esc(who)}</div>
      <div style="font-size:13px;color:${C.mute};margin-bottom:12px">${esc([f.tech_name, f.trade, f.job_number ? `Job #${f.job_number}` : null, when].filter(Boolean).join(' · '))}</div>
      <div>
        ${chip(`Priority ${f.score ?? '—'} of 5`, f.score >= 4 ? '#FEE2E2' : '#FEF3C7', f.score >= 4 ? '#B91C1C' : C.warn)}
        ${chip(TYPE_LABEL[f.followup_type] || 'Follow-up', '#E0F2FE', '#075985')}
        ${cost > 0 ? chip(`Quoted $${Math.round(cost).toLocaleString('en-US')}`, '#DCFCE7', C.good) : ''}
        ${c.follow_up_time ? chip(`When: ${c.follow_up_time}`, '#F1F5F9', C.ink) : ''}
      </div>
      ${block('Best way to follow up', c.follow_up_summary, { box: true })}
      ${block('What was quoted', [c.scope_cost_summary, c.pricing_options].filter(Boolean).join('\n\n'))}
      ${block('Why it didn’t close', [c.objection_summary, objections ? `Main objection: ${objections}` : ''].filter(Boolean).join('\n'))}
      ${block('Financing offered', c.financing)}
      ${block(`Ready-to-send text (from ${first})`, c.FollowupTextHeader, { box: true })}
      ${block(`Ready-to-send email (from ${first})`, c.FollowupEmailHeader, { box: true })}
      <div style="margin-top:20px">
        ${btn(stUrl, 'Open the job in ServiceTitan', true)}
        ${btn(rec?.web_url, 'Listen in Field Pro')}
      </div>
      <div style="margin-top:18px;padding-top:12px;border-top:1px solid ${C.line};font-size:11.5px;color:${C.mute};line-height:1.6">
        Field Pro flagged this from ${esc(first)}’s in-home recording. Sent to Brandyn${manager?.name ? `, ${esc(manager.name)} (${esc(f.trade)})` : ''} and ${esc(f.tech_name || 'the tech')}.
        Drafts are Siro’s suggestion — read before sending. When it’s handled, mark it done in Field Pro → Re-Engage.
      </div>
    </div>`
    const subject = `Re-Engage: ${who}${f.job_number ? ` (job #${f.job_number})` : ''} — ${first}, priority ${f.score ?? '?'}/5`
    return { subject, html }
  }

  let _mailBusy = false
  async function sendReengageEmails({ force = false } = {}) {
    if (_mailBusy || !sendResend) return null
    _mailBusy = true
    try {
      const { cfg, mgrs } = await emailConfig()
      if (!cfg.enabled) return { skipped: 'disabled' }
      if (!force && cfg.startAt && Date.now() < Date.parse(cfg.startAt)) return { skipped: 'before start' }
      const h = denverHour()
      if (!force && (h < 7 || h >= 19)) return { skipped: 'outside 7 AM–7 PM Denver' }
      let q = supabase.from('siro_followups').select('*').is('emailed_at', null).eq('status', 'TO_DO')
        .gte('score', Number(cfg.minScore) || 3).lt('email_attempts', 3).order('created_at', { ascending: true }).limit(15)
      // Never the backlog: only follow-ups from `since` (default: the switch-on time).
      const since = cfg.since || cfg.startAt
      if (!since) return { skipped: 'no since/startAt set' }
      q = q.gte('created_at', since)
      const { data: rows } = await q
      let sent = 0
      for (const f of (rows || [])) {
        // Replica-safe claim: only one server sends a given follow-up.
        const { data: claimed } = await supabase.from('siro_followups').update({ emailed_at: new Date().toISOString(), email_attempts: (f.email_attempts || 0) + 1 })
          .eq('id', f.id).is('emailed_at', null).select('id')
        if (!claimed?.length) continue
        try {
          const { data: rec } = await supabase.from('siro_recordings').select('web_url').eq('id', f.recording_id).maybeSingle()
          const r = await recipientsFor(f, cfg, mgrs)
          if (!r.to.length) throw new Error('no recipients')
          const { subject, html } = reengageEmail(f, rec, r)
          await sendResend({ to: r.to, subject, html })
          await supabase.from('siro_followups').update({ email_to: r.to, email_error: null }).eq('id', f.id)
          sent++
        } catch (e) {
          // Release for a retry (3 tries max) — a flaky minute shouldn't lose a lead.
          await supabase.from('siro_followups').update({ emailed_at: null, email_error: String(e.message).slice(0, 300) }).eq('id', f.id)
          console.warn('re-engage email:', e.message)
        }
      }
      if (sent) console.log(`field pro: sent ${sent} Re-Engage email(s)`)
      return { sent }
  ...
  return {
    syncRecordings, refreshFollowupStatuses, syncTechScorecards, sendReengageEmails, monthScores, coaching, digestFacts,
    recordingScorecard, reengageEmail, hasReadToken, siroUsers,
  }
}
