...
export async function gatherDigestFacts({ stGet, stPageAll, supabase, tenantId, dateStr, getBoard3Day }) {
  ...
  const callCenter = { answered: 0, abandoned: 0, avgTalkSec: null, bookedByCsr: 0, bookingPct: null, qaAvg: null, qaCount: 0, outboundDials: 0, byRep: [], voicemails: 0, coaching: [] }
  try {
    ...
    const byRep = new Map()
    const rep = (n) => {
      const cur = byRep.get(n) || { name: n, answered: 0, booked: 0, unbooked: 0, outbound: 0, talks: [] }
    ...
    for (const c of telecom) {
      const lc = c.leadCall || c
      const agent = (lc.agent || {}).name || (lc.createdBy || {}).name || null
    ...
    callCenter.byRep = [...byRep.values()].map(r => ({
      ...
      outbound: r.outbound,
    })).filter(r => r.answered).sort((a, b) => b.answered - a.answered)
    ...
    const obOnly = [...byRep.values()].filter(r => !r.answered && r.outbound)
    callCenter.outboundOnly = { people: obOnly.length, calls: obOnly.reduce((a, r) => a + r.outbound, 0) }

    const [{ data: evals }, { data: recs }] = await Promise.all([
      supabase.from('call_evaluations').select('rep, pct, summary, scores').gte('created_at', startIso).lt('created_at', endIso),
      supabase.from('call_recordings').select('rep, direction, duration, st_job_id').gte('call_started_at', startIso).lt('call_started_at', endIso),
    ])
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:720px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    ${section('Call center', `
      <table style="width:100%;border-collapse:separate;border-spacing:8px 0"><tr>
        ...
        ${card('Avg talk', f.callCenter.avgTalkSec == null ? '—' : `${Math.round(f.callCenter.avgTalkSec / 60)}m`, f.callCenter.outboundDials ? `${f.callCenter.outboundDials} outbound dials` : '')}
        ${card('Call QA', f.callCenter.qaAvg == null ? '—' : `${f.callCenter.qaAvg}%`, `${f.callCenter.qaCount} scored in Andi`)}
      </tr></table>
      ${f.callCenter.byRep.length ? `<div style="margin-top:12px">${table(['CSR', 'Answered', 'Booked', 'Book rate', 'Avg talk', 'Outbound'],
        f.callCenter.byRep.map(r => [esc(r.name), String(r.answered), String(r.booked), r.bookRate == null ? '—' : pct(r.bookRate), r.avgTalkSec == null ? '—' : `${Math.round(r.avgTalkSec / 60)}m`, String(r.outbound)]))}</div>` : ''}
      <div style="font-size:11px;color:${C.mute};margin-top:8px">Per-CSR stats from ServiceTitan call records; QA scores from Andi evaluations (calls taken in Andi only, for now).${f.callCenter.outboundOnly?.people ? ` ${f.callCenter.outboundOnly.calls} more outbound calls by ${f.callCenter.outboundOnly.people} field/other staff not shown.` : ''}</div>`)}
