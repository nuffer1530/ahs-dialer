...
export async function gatherDigestFacts({ stGet, stPageAll, supabase, tenantId, dateStr, getBoard3Day }) {
  ...
  try {
    ...
    const [jtRes, ...batches] = await Promise.all([
  ...
  const withEstActivity = new Set([...presentedJobs, ...convJobs])
  const closeRate = {
  ...
  for (const t of TRADES) {
    const convT = new Set([...(soldJobsByTrade[t] || []), ...(rev90ByTrade[t] || [])])
    const act = new Set([...(presentedByTrade[t] || []), ...convT])
    closeRate.byTrade[t] = { presented: act.size, sold: convT.size, rate: act.size ? convT.size / act.size : null }
  }
