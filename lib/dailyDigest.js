...
// Invoice line used ONLY by the money-left-behind section (a visit that
// invoiced this much didn't need an estimate written). Close rate itself is
// ServiceTitan's Sales close from lib/salesClose.js and never looks at invoices.
...
export async function gatherDigestFacts({ stGet, stPageAll, supabase, tenantId, dateStr, getBoard3Day, getFieldPro }) {
  ...
  let [estSold, estCreated, invoices, appts, telecom, memberships, reviews] = await Promise.all([
  ...
  // out of booking %, the CSR table and the marketing rates (Brandyn, Sep 14).
  ...
  // ST threshold conversions: a job that invoiced >= $89 today converted even
  // without a Sold estimate (ST's configured threshold; techs invoice repairs
  ...
  const rev90ByTrade = {}
  {
    ...
    for (const i of invoices) {
      const jid = (i.job || {}).id
      if (jid) byJob.set(jid, (byJob.get(jid) || 0) + (Number(i.subTotal) || 0))
    }
    for (const i of invoices) {
      const jid = (i.job || {}).id
      if (!jid || (byJob.get(jid) || 0) < CONV_THRESHOLD || rev90.has(jid)) continue
      rev90.add(jid)
      const t = tradeOf((i.businessUnit || {}).name)
      if (t) (rev90ByTrade[t] = rev90ByTrade[t] || new Set()).add(jid)
    }
  }

  // Installs fulfill an already-sold estimate — NEVER opportunities (Brandyn,
  // Sep 2026; same rule as the TV boards and leadership report). Drop
  // install-typed jobs from every close-rate set; sales dollars, revenue, and
  // job counts elsewhere still include them.
  // Maintenance tune-ups auto-generate option sheets in ST and are not
  // opportunities either: only sales-shaped job-type categories (repair /
  // other / free estimate; unmapped types still count) stay in the sets —
  // the same rule the TV boards and the leadership report use.
  const SALES_OPP_CATS = new Set(['repair', 'other', 'free_estimate'])
  const installJobs = new Set(), notOppJobs = new Set()
  try {
    const ids = [...new Set([...presentedJobs, ...soldJobIds, ...rev90])]
    const [jtRes, spiffRes, ...batches] = await Promise.all([
      safe(stGet(`/jpm/v2/tenant/${tenantId}/job-types?pageSize=500`), null),
      supabase.from('job_type_spiffs').select('st_job_type_id, category').then(r => r.data || []).catch(() => []),
      ...Array.from({ length: Math.ceil(ids.length / 50) }, (_, i) =>
        safe(stGet(`/jpm/v2/tenant/${tenantId}/jobs?ids=${ids.slice(i * 50, i * 50 + 50).join(',')}&pageSize=50`), null)),
    ])
    const jtName = new Map((jtRes?.data || []).map(t => [String(t.id), t.name || '']))
    const catOf = new Map((spiffRes || []).map(r => [String(r.st_job_type_id), r.category || null]))
    for (const b of batches) for (const j of (b?.data || [])) {
      if (/install/i.test(jtName.get(String(j.jobTypeId)) || '')) installJobs.add(j.id)
      const c = catOf.get(String(j.jobTypeId))
      if (c != null && !SALES_OPP_CATS.has(c)) notOppJobs.add(j.id)
    }
    for (const set of [presentedJobs, soldJobIds, rev90, ...Object.values(presentedByTrade), ...Object.values(soldJobsByTrade), ...Object.values(rev90ByTrade)]) {
      for (const j of installJobs) set.delete(j)
      for (const j of notOppJobs) set.delete(j)
    }
  } catch (e) { console.warn('digest install filter:', e.message) }

  // Opportunity = estimate activity OR invoiced >= $89; converted = Sold
  // estimate OR invoiced >= $89 — ST's threshold semantics.
  // Scoped to jobs that RAN yesterday (computed below, once the ran set exists).
  const convJobs = new Set([...soldJobIds, ...rev90])
  const closeRate = { presented: 0, sold: 0, rate: null, byTrade: {} }
  ...
  try {
    const [t, bus] = await Promise.all([
      ...
      safe(stGet(`/settings/v2/tenant/${tenantId}/business-units?pageSize=200`), null),
    ])
  ...
  // Company + trade close = ServiceTitan's Sales close for the day
  // (lib/salesClose.js — the Technician Scorecard number), attributed to the
  // tech's business unit like the TV rosters.
  let closeResult = null
  try {
    const techTrade = (tid) => { const i = techNames.get(String(tid)) || {}; return tradeOf(i.bu) || buTrade.get(String(i.buId)) || null }
    closeResult = computeSalesClose(await fetchSalesCloseInputs({ stGet, stPageAll, tenantId, startIso, endIso }))
    const shape = (c) => ({ presented: c?.opps || 0, sold: c?.closed || 0, rate: c?.rate ?? null })
    Object.assign(closeRate, shape(rollupSalesClose(closeResult).all))
    const byTrade = rollupSalesClose(closeResult, {
    ...
    for (const t of TRADES) closeRate.byTrade[t] = shape(byTrade[t])
  } catch (e) { console.warn('digest sales close:', e.message) }
  ...
  for (const [techId, jobs] of techJobs) {
    ...
    // Per-tech close = their line on ST's Technician Scorecard for the day.
  ...
  // Install jobs are excluded — under the installs-aren't-opportunities rule
  // they'd otherwise be mislabeled "no estimate written".
  const ranJobIds = new Set()
  for (const jobs of techJobs.values()) for (const j of jobs) if (!installJobs.has(j)) ranJobIds.add(j)
  ...
  const leftBehind = {
    ...
    noEstimateWritten: [...ranJobIds].filter(j => !presentedJobs.has(j) && !rev90.has(j)).length,
    presentedNotSold: ranWithEstimate.filter(j => !convJobs.has(j)).length,
  ...
  // Overnight disposition corrections (the 1 AM sweep) — surfaced so Brittany
  // and Deanna can audit every flip instead of trusting the AI blind.
  let dispositions = null
  try {
    const { data } = await supabase.from('st_disposition_log')
      .select('rep, phone, customer, from_type, to_type, reason')
      .eq('day', dateStr).order('created_at', { ascending: true }).limit(100)
    if (data?.length) {
      dispositions = {
        count: data.length,
        booked: data.filter(r => r.to_type === 'Booked').length,
        excused: data.filter(r => r.to_type === 'Excused').length,
        rows: data,
      }
    }
  } catch (e) { console.warn('digest dispositions:', e.message) }

  // ── Field Pro (Siro): yesterday's in-home recordings, scores, Re-Engage ──
  let fieldPro = null
  if (typeof getFieldPro === 'function') {
    try { fieldPro = await getFieldPro(dateStr, technicians) } catch (e) { console.warn('digest field pro:', e.message) }
  }
...
async function generateInsights(facts, anthropicKey) {
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-5', max_tokens: 3500,
...
Also write FIELD TECH coaching from Field Pro (Siro in-home call recordings, scored 0-100 on the trade's scorecard: 80+ strong, 50-79 developing, under 50 a gap), given separately as "Field Pro". For each tech in fieldPro.byTech, ONE thing to praise and ONE step to coach, grounded in their numbers (avgScore, monthWeakestSteps with scores) and Siro's growth notes — e.g. "Price Presentation is at 23/100 this month; walk the options sheet line by line." Behavior, never the person; one recording = say it's one call. If fieldPro.notRecording lists techs who ran jobs without recording, you may turn that into an insight with an action.`,
        tools: [{
          ...
          input_schema: {
            ...
            properties: {
              ...
              fieldProCoaching: {
                type: 'array',
                description: 'One entry per technician in Field Pro byTech (yesterday\'s in-home recordings).',
                items: {
                  type: 'object',
                  properties: {
                    name: { type: 'string' },
                    working: { type: 'string', description: 'What they did well on their recorded calls, with the number.' },
                    coach: { type: 'string', description: 'The one step to coach, with its score.' },
                  },
        ...
        messages: [{ role: 'user', content: `Yesterday's numbers:\n${JSON.stringify({ ...facts, fieldPro: undefined }).slice(0, 22000)}${facts.fieldPro ? `\n\nField Pro:\n${JSON.stringify(facts.fieldPro).slice(0, 7000)}` : ''}` }],
      }),
    })
    if (!r.ok) return []
    const out = (await r.json())?.content?.find(c => c.type === 'tool_use')?.input
    return out ? { headline: out.headline, insights: (out.insights || []).slice(0, 6), csrCoaching: (out.csrCoaching || []).slice(0, 8), fieldProCoaching: (out.fieldProCoaching || []).slice(0, 25) } : []
  } catch (e) {
...
// Field Pro block: yesterday's recorded in-home calls (Siro), per tech, the
// Re-Engage leads they produced, and the month-to-date score by trade.
function fieldProHtml(fp, ai) {
  const tone = (v) => (v == null ? C.mute : v >= 80 ? C.good : v >= 50 ? C.warn : C.bad)
  const sc = (v) => (v == null ? '—' : `<span style="color:${tone(v)};font-weight:700">${v}</span>`)
  const coach = ai?.fieldProCoaching || []
  return `
    <table style="width:100%;border-collapse:separate;border-spacing:8px 0"><tr>
      ${card('Recorded calls', String(fp.recordings), `${fp.techs} tech${fp.techs === 1 ? '' : 's'} recording`)}
      ${card('Avg Field Pro score', fp.avgScore == null ? '—' : `${fp.avgScore}/100`, `${fp.scored} scored`)}
      ${card('Won on the call', String(fp.wonOnCall), 'per Siro')}
      ${card('Re-Engage leads', String(fp.reengage.new), fp.reengage.quoted ? `${money(fp.reengage.quoted)} quoted` : (fp.reengage.highPriority ? `${fp.reengage.highPriority} high priority` : 'new yesterday'))}
    </tr></table>
    ${fp.byTech.length ? `<div style="margin-top:12px">${table(['Technician', 'Trade', 'Recorded', 'Avg score', 'Won', 'Jobs ran', 'Month score'],
      fp.byTech.map(t => [esc(t.name), esc(t.trade || '—'), String(t.recordings), sc(t.avgScore), String(t.won), t.jobsRan == null ? '—' : String(t.jobsRan), sc(t.monthScore)]))}</div>` : ''}
    ${fp.notRecording?.length ? `<div style="font-size:12px;color:${C.warn};margin-top:10px"><b>Ran jobs without recording:</b> ${fp.notRecording.map(t => `${esc(t.name)} (${t.jobsRan} job${t.jobsRan === 1 ? '' : 's'})`).join(' · ')}</div>` : ''}
    ${coach.length ? `<div style="margin-top:12px">${coach.map(c => `
    ...
    ${fp.monthByTrade.some(t => t.score != null) ? `<div style="font-size:11.5px;color:${C.mute};margin-top:8px">Month to date by trade: ${fp.monthByTrade.filter(t => t.score != null).map(t => `${esc(t.trade)} <b style="color:${tone(t.score)}">${t.score}</b> (${t.scored} calls)`).join(' · ')}</div>` : ''}
    <div style="font-size:11px;color:${C.mute};margin-top:8px">Field Pro (Siro) records the tech's conversation in the home and scores it 0–100 on the trade's scorecard (80+ strong, 50–79 developing, under 50 a gap). Re-Engage leads go out in a 7 AM roundup — each tech gets their own, operations managers their trade's, Brandyn all of them. Full coaching cards: Andi → Team → Technicians.</div>`
}
...
export function renderDigestHtml(f, ai) {
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:720px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    <table style="width:100%;border-collapse:separate;border-spacing:8px 0"><tr>
      ...
      ${card('Close rate', pct(f.closeRate.rate), `${f.closeRate.sold} of ${f.closeRate.presented} opportunities`)}
    ...
    ${f.fieldPro ? section('Field Pro — in-home calls', fieldProHtml(f.fieldPro, ai)) : ''}
    ...
    ${f.dispositions ? section('Dispositions corrected overnight', `
      <div style="font-size:13px;color:${C.ink};margin-bottom:8px">
        ${f.dispositions.count} call${f.dispositions.count === 1 ? '' : 's'} re-classified in ServiceTitan
        (${f.dispositions.booked} to Booked, ${f.dispositions.excused} to Excused). Real unbooked leads were left alone.
      </div>
      ${table(['CSR', 'Caller', 'Change', 'Why'],
        f.dispositions.rows.map(r => [
          esc(r.rep || '—'),
          esc(r.customer || r.phone || '—'),
          `${esc(r.from_type)} → <b>${esc(r.to_type)}</b>`,
          esc(r.reason || ''),
        ]))}
      <div style="font-size:11px;color:${C.mute};margin-top:8px">Nightly AI sweep of yesterday's Unbooked calls: obvious non-leads (solicitors, wrong numbers, existing-job logistics) are excused, calls that actually booked are credited. Only high-confidence calls are changed, each call at most once, and every change is listed here for audit.</div>`) : ''}
    ...
    <div style="margin-top:26px;padding-top:12px;border-top:1px solid ${C.line};font-size:11px;color:${C.mute};line-height:1.6">
      Sales = estimates marked Sold in ServiceTitan · Revenue = invoiced subtotals · Close rate = ServiceTitan's Sales close (the Technician Scorecard number): completed chargeable sales jobs, closed when sold estimates first reach the $90 sold threshold ·
...
export async function buildDailyDigest(deps) {
  // deps.facts: already-gathered facts (the server shares one gather per day
  // between this email and the Home morning briefs).
  const facts = deps.facts || await gatherDigestFacts(deps)
