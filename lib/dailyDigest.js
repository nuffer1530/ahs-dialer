...
export async function gatherDigestFacts({ stGet, stPageAll, supabase, tenantId, dateStr, getBoard3Day }) {
  ...
  // Company + trade close, ServiceTitan's way: over jobs that RAN yesterday,
  // attributed to the tech's business unit like the TV rosters.
  {
    const techTrade = (tid) => { const i = techNames.get(String(tid)) || {}; return tradeOf(i.bu) || buTrade.get(String(i.buId)) || null }
    const stClose = (ranSet) => {
      let opps = 0, won = 0
      for (const j of ranSet) {
        if (!presentedJobs.has(j) && !convJobs.has(j)) continue
        opps++; if (convJobs.has(j)) won++
      }
      return { presented: opps, sold: won, rate: opps ? won / opps : null }
    }
    const ranOf = (trade) => { const out = new Set(); for (const [tid, jobs] of techJobs) { if (trade && techTrade(tid) !== trade) continue; for (const j of jobs) out.add(j) } return out }
    Object.assign(closeRate, stClose(ranOf(null)))
    for (const t of TRADES) closeRate.byTrade[t] = stClose(ranOf(t))
  }
  ...
  for (const [techId, jobs] of techJobs) {
    ...
    const soldJobsSet = new Set([...jobs].filter(j => soldJobIds.has(j) || rev90.has(j)))
...
export function renderDigestHtml(f, ai) {
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:720px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    <div style="margin-top:26px;padding-top:12px;border-top:1px solid ${C.line};font-size:11px;color:${C.mute};line-height:1.6">
      Sales = estimates marked Sold in ServiceTitan · Revenue = invoiced subtotals · Close rate = converted ÷ opportunities, ServiceTitan's rule: a ran sales-type job (no installs, no maintenance) with an estimate or $89+ invoiced; converted = sold or $89+ invoiced ·
