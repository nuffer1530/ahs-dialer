...
export async function gatherDigestFacts({ stGet, stPageAll, supabase, tenantId, dateStr, getBoard3Day }) {
  ...
  // ST threshold conversions: a job that invoiced >= $90 today converted even
  ...
  const rev90ByTrade = {}
  {
    ...
    for (const i of invoices) {
      const jid = (i.job || {}).id
      if (!jid || (byJob.get(jid) || 0) < 90 || rev90.has(jid)) continue
      rev90.add(jid)
  ...
  // Opportunity = estimate activity OR invoiced >= $90; converted = Sold
  // estimate OR invoiced >= $90 — ST's threshold semantics, per Brandyn.
...
export function renderDigestHtml(f, ai) {
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:720px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    <div style="margin-top:26px;padding-top:12px;border-top:1px solid ${C.line};font-size:11px;color:${C.mute};line-height:1.6">
      Sales = estimates marked Sold in ServiceTitan · Revenue = invoiced subtotals · Close rate = converted ÷ opportunities, ServiceTitan's rule: a ran sales-type job (no installs, no maintenance) with an estimate or $90+ invoiced; converted = sold or $90+ invoiced ·
