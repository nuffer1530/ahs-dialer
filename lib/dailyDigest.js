...
export async function gatherDigestFacts({ stGet, stPageAll, supabase, tenantId, dateStr, getBoard3Day }) {
  ...
  const callCenter = { answered: 0, abandoned: 0, avgWaitSec: null, avgTalkSec: null, bookedByCsr: 0, qaAvg: null, qaCount: 0, outboundDials: 0, byRep: [], voicemails: 0, coaching: [] }
  try {
    const [{ data: tasks }, { data: bookings }, { data: evals }, { data: logs }, { data: recs }] = await Promise.all([
