...
// Like zScores, but rows missing the value sit at 0 (bench-neutral) instead
// of dragging the mean — most techs have no review rating yet.
function zScoresSparse(rows, key) {
  const rated = rows.filter(r => r[key] != null)
  if (rated.length < 2) return rows.map(() => 0)
  const vals = rated.map(r => Number(r[key]))
  const mean = vals.reduce((a, b) => a + b, 0) / vals.length
  const sd = Math.sqrt(vals.reduce((a, b) => a + (b - mean) ** 2, 0) / vals.length) || 1
  return rows.map(r => (r[key] == null ? 0 : (Number(r[key]) - mean) / sd))
}
...
export function computeBattingOrder(data, weights = DEFAULT_WEIGHTS, opts = {}) {
  ...
  const all = [...groups.values()].map(g => ({
    ...
    totalSold: g.revenue,
  })).map(r => ({ ...r, expectedValue: (r.closeRate / 100) * r.avgSale, reviewRating: reviews[String(r.techId)] ?? null }))
  ...
  for (const [bu, rows] of byBU) {
    ...
    const zr = zScoresSparse(eligible, 'reviewRating')
