...
export const DEFAULT_WEIGHTS = { expectedValue: 55, closeRate: 20, membership: 15, reviews: 10 }
...
export function computeBattingOrder(data, weights = DEFAULT_WEIGHTS, opts = {}) {
  const { jobs = [], assignments = [], estimates = [], memberships = [], technicians = [], reviews = [], jobTypes = [] } = data
  // Installs are never opportunities (Brandyn, Sep 2026) — the sale happened
  // on the earlier call; the install visit is fulfillment.
  const _jtName = new Map(jobTypes.map(t => [t.id, t.name || '']))
  const isInstallJob = (j) => /install/i.test(_jtName.get(j?.jobTypeId) || '')
  ...
  for (const [jobId, options] of optionsByJob) {
    ...
    if (!g) continue
    if (isInstallJob(jobById.get(jobId))) continue   // install ≠ opportunity
  ...
  // ST Marketing Reputation reviews, matched by technicianId (~2/3 of Google
  // reviews carry the match). Scored like memberships: recency-weighted
  // 5-star reviews per weighted job run.
  const revByTech = new Map()
  for (const r of reviews) {
    const tid = r?.technicianId
    if (!tid) continue
    const e = revByTech.get(tid) || { w5: 0, n: 0, n5: 0, sum: 0 }
    if ((r.rating || 0) === 5) { e.w5 += recencyWeight(r.publishDate, now); e.n5 += 1 }
    e.n += 1
  ...
  const jobsWByKey = new Map([...groups.entries()].map(([k, g]) => [k, g.jobsW]))
  const all = [...groups.values()].map(g => ({
    ...
    const rv = revByTech.get(r.techId)
    return {
      ...r,
      expectedValue: (r.closeRate / 100) * r.avgSale,
      reviewPct: r._jobsW ? ((rv?.w5 || 0) / r._jobsW) * 100 : 0,
      reviewsN: rv?.n || 0,
      reviewsN5: rv?.n5 || 0,
      reviewAvg: rv?.n ? rv.sum / rv.n : null,
      reviewW5: rv?.w5 || 0,
    }
  })
  ...
  for (const [bu, rows] of byBU) {
    ...
    // Small-sample shrinkage: per-opportunity stats are noise at 2-3 opps —
    // a tech who closed both of his two chances is not a 100% closer (Josh
    // Martinez ranked #1 on exactly that). Pull rate stats toward the bench
    // mean with a prior worth PRIOR_OPPS opportunities; real volume earns
    // full voice, tiny samples read as "about average until proven".
    const PRIOR_OPPS = 8
    const meanOf = (k) => eligible.reduce((a, r) => a + r[k], 0) / eligible.length
    const mEV = meanOf('expectedValue'), mCR = meanOf('closeRate')
    for (const r of eligible) {
      const w = r.opportunities || 0
      r.evShrunk = (w * r.expectedValue + PRIOR_OPPS * mEV) / (w + PRIOR_OPPS)
      r.crShrunk = (w * r.closeRate + PRIOR_OPPS * mCR) / (w + PRIOR_OPPS)
    }
    const ze = zScores(eligible, 'evShrunk')
    const zc = zScores(eligible, 'crShrunk')
    const zm = zScores(eligible, 'membershipPct')
    const zr = zScores(eligible, 'reviewPct')
    const scored = eligible.map((r, i) => ({
      ...r,
      // weights.reviews may be absent in weight sets saved before it existed.
      score: (weights.expectedValue * ze[i] + weights.closeRate * zc[i] + weights.membership * zm[i] + (weights.reviews || 0) * zr[i]) / 100,
    })).sort((a, b) => b.score - a.score)

    // Tiers require MATERIAL separation, not just a rank.
    //
    // A pure percentile curve always manufactures a red tech, even on a bench
    // that's statistically identical. Real example: Plumbing - Sales had
    // Preston at $3,853/opp tagged "not on high-value" against Clayton at
    // $4,164 — a 7% gap, and Preston actually closed at a HIGHER rate over 23
    // more jobs. That label is both wrong and damaging: dispatch stops using a
    // tech who is in fact interchangeable with their best.
    //
    // So tier on the distance from the bench MEDIAN in expected value (the
    // money number, and interpretable), and let a tight bench be all-Standard.
    const evs = scored.map(r => r.evShrunk).sort((a, b) => a - b)
    const mid = evs.length % 2
      ? evs[evs.length >> 1]
      : (evs[(evs.length >> 1) - 1] + evs[evs.length >> 1]) / 2
    const GREEN_RATIO = 1.10   // >=10% above the bench median
    const RED_RATIO = 0.75     // <=25% below it
    const MIN_BENCH_FOR_RED = 3  // on a two-man bench somebody is always "last"

    const spread = evs.length && evs[evs.length - 1]
      ? (evs[evs.length - 1] - evs[0]) / evs[evs.length - 1]
      : 0

    scored.forEach((r, i) => {
      const ratio = mid ? r.evShrunk / mid : 1
      let tier = 'yellow'
      if (ratio >= GREEN_RATIO && r.score > 0) tier = 'green'
      else if (ratio <= RED_RATIO && scored.length >= MIN_BENCH_FOR_RED) tier = 'red'
      ranked.push({ ...r, tier, rank: i + 1, benchSpread: spread, benchMedianEv: mid })
    })
  }
...
// ── Job-type batting order ──────────────────────────────────────────────────
// "Who is my best guy on tanked water heaters?" Same opportunity basis as the
// team ranking: one opportunity = one job, however many options were presented.
//
// Only job types where opportunities actually existed appear — an opportunity
// requires an estimate, so routine no-estimate work drops out naturally rather
// than needing a hand-maintained exclusion list. Install benches are excluded
// via NON_DISPATCH_TEAM like everywhere else.
//
// Sample sizes here are SMALL by nature (a tech might see one job type a
// handful of times in 45 days), so this reports opportunity counts loudly and
// marks anything thin. It is a lookup tool, not a performance review.
const JOBTYPE_MIN_OPPS = 3      // below this, don't show at all
const JOBTYPE_THIN_OPPS = 8     // below this, flag as low confidence

export function computeJobTypeOrder(data, opts = {}) {
  const { jobs = [], assignments = [], estimates = [], technicians = [], jobTypes = [] } = data
  const now = opts.now ?? Date.now()

  const teamOf = new Map(technicians.map(t => [t.id, (t.team || 'Unassigned').trim()]))
  const jtName = new Map(jobTypes.map(t => [t.id, t.name || '']))
  const jobById = new Map(jobs.map(j => [j.id, j]))
  const techOfJob = new Map()
  for (const a of assignments) {
    if (a?.jobId && a?.technicianId) techOfJob.set(a.jobId, { id: a.technicianId, name: a.technicianName || String(a.technicianId) })
  }
  ...
  const agg = new Map()   // `${techId}|${jobTypeId}`
  for (const [jobId, options] of optionsByJob) {
    const j = jobById.get(jobId)
    const tech = techOfJob.get(jobId)
    if (!j || !tech) continue
    const team = teamOf.get(tech.id) || 'Unassigned'
    if (NON_DISPATCH_TEAM.test(team)) continue
    const jtId = j.jobTypeId
    const name = jtName.get(jtId) || ''
    if (!name || /install/i.test(name)) continue     // installs aren't opportunities to win

    const k = `${tech.id}|${jtId}`
    if (!agg.has(k)) {
      agg.set(k, { techId: tech.id, techName: tech.name, team, jobTypeId: jtId, jobType: name, opps: 0, won: 0, revenue: 0 })
    }
    const g = agg.get(k)
    g.opps += 1
    const win = options.filter(o => nameOf(o.status) === 'Sold')
    if (win.length) {
      g.won += 1
      g.revenue += win.reduce((a, o) => a + Number(o.subtotal || 0), 0)
    }
  }

  return [...agg.values()]
    .filter(g => g.opps >= JOBTYPE_MIN_OPPS)
    .map(g => {
      const closeRate = g.opps ? (g.won / g.opps) * 100 : 0
      const avgSale = g.won ? g.revenue / g.won : 0
      return {
        ...g,
        closeRate,
        avgSale,
        expectedValue: (closeRate / 100) * avgSale,
        thin: g.opps < JOBTYPE_THIN_OPPS,
      }
    })
    .sort((a, b) => a.jobType.localeCompare(b.jobType) || b.expectedValue - a.expectedValue)
}
