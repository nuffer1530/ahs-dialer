...
//  - OPPORTUNITY CLOSE RATE: estimates on the tech's jobs that are Sold ÷ all
//    estimates on those jobs. NOT estimate.soldBy — that field only exists on
//    sold estimates, so it yields a numerator with no denominator (every tech
//    would score 100%). Tech resolved job → appointment → assignment.
//  - AVERAGE SALE: MEDIAN value of the tech's SOLD ESTIMATES.
...
//    Joseph Friel at $89 while he had actually sold $437k in 45 days. Measuring
//    invoices measures who collects, not who sells.
...
export function computeBattingOrder(data, weights = DEFAULT_WEIGHTS, opts = {}) {
  const { jobs = [], assignments = [], estimates = [], memberships = [], businessUnits = [] } = data
  ...
  const touch = (techId, techName, bu) => {
    ...
    if (!groups.has(k)) {
      groups.set(k, { techId, techName, businessUnit: bu, jobsRaw: 0, jobsW: 0, estW: 0, soldW: 0, sales: [] })
    }
    return groups.get(k)
  }
  ...
  for (const e of estimates) {
    ...
    g.estW += w
    if (nameOf(e.status) === 'Sold') {
      g.soldW += w
      const v = Number(e.subtotal || 0)
      if (v > 0) g.sales.push(v)   // what they SOLD, not what got invoiced
    }
  }
  ...
  const all = [...groups.values()].map(g => ({
    ...
    jobs: g.jobsRaw,
    closeRate: g.estW ? (g.soldW / g.estW) * 100 : 0,
    avgSale: median(g.sales),
    membershipPct: g.jobsW ? ((memByTech.get(g.techId) || 0) / g.jobsW) * 100 : 0,
    totalSold: g.sales.reduce((a, b) => a + b, 0),
