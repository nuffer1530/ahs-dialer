...
async function writeBrief(scope, slice, anthropicKey) {
...
- Plain words a manager scans in ten seconds. No fluff, no congratulations, no restating every field.
...
- Yesterday can be a weekend or a light day; say so if the volume is too small to read into.`,
        tools: [{
          ...
          input_schema: {
            type: 'object',
            properties: {
              headline: { type: 'string', description: 'One sentence, the single most important thing for this reader this morning, number first. Max ~18 words.' },
              yesterday: { type: 'array', items: { type: 'string' }, description: '2–4 one-line takeaways from yesterday, each with its number.' },
              today: { type: 'array', items: { type: 'string' }, description: '1–3 things to do today, each concrete (who/what/where).' },
              coach: {
                type: 'object',
                description: 'The one person to coach first and on what. Omit fields you have no evidence for.',
                properties: {
                  who: { type: 'string' }, focus: { type: 'string', description: 'The behavior or step, short.' },
                  why: { type: 'string', description: 'The evidence, with numbers.' }, drill: { type: 'string', description: 'What to do in the coaching session / ride-along.' },
                },
                required: ['who', 'focus'],
              },
            },
        ...
        messages: [{ role: 'user', content: `Scope: ${slice.scope}. Yesterday = ${slice.date}.\n\n${JSON.stringify(slice).slice(0, 24000)}` }],
      }),
    })
    const d = await r.json()
    if (!r.ok) throw new Error(d?.error?.message || `Anthropic ${r.status}`)
    const out = (d.content || []).find(c => c.type === 'tool_use')?.input
    if (!out || typeof out.headline !== 'string') throw new Error('No brief returned')
    ...
    const c = out.coach && typeof out.coach === 'object' && out.coach.who ? {
      who: String(out.coach.who), focus: String(out.coach.focus || ''), why: out.coach.why ? String(out.coach.why) : null, drill: out.coach.drill ? String(out.coach.drill) : null,
    } : null
    return { headline: out.headline.trim(), yesterday: list(out.yesterday), today: list(out.today), coach: c }
...
export function createHomeBriefs(deps) {
  ...
  async function read(dateStr, keys) {
    ...
    return Object.fromEntries((data || []).map(r => [r.scope, { ...r.brief, writtenAt: r.created_at }]))
  }
  ...
  function ensure(dateStr, key) {
    ...
    const p = (async () => {
      ...
      return brief
    })().catch(e => { console.warn(`home brief ${key}:`, e.message); return null })
  ...
  async function forScopes(dateStr, keys) {
    ...
    return { date: dateStr, briefs: have, pending: keys.filter(k => !have[k]) }
  }
