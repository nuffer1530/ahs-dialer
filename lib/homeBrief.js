...
- Yesterday can be a weekend or a light day; say so if the volume is too small to read into.`,
    ...
    const list = (v, n) => (Array.isArray(v) ? v : typeof v === 'string' ? v.split(/\n+/) : []).map(x => String(x).replace(/^[-•\s]+/, '').replace(/\.$/, '').trim()).filter(Boolean).slice(0, n)
    const c = out.coach && typeof out.coach === 'object' && out.coach.who ? { who: String(out.coach.who), focus: String(out.coach.focus || '') } : null
    return { v: 2, headline: out.headline.trim().replace(/\.$/, ''), yesterday: list(out.yesterday, 3), today: list(out.today, 2), coach: c }
