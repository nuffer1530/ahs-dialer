...
// ServiceTitan's conversion line: an invoice at or above this converts the job.
// $89 (the consult/trip fee) converts in ST — proven against ST's own Electrical
// close for the week of Sep 14, 2026 (26/36 = 72.2% only at >= $89). Keep in
// step with TV_CONV_THRESHOLD in server.js.
const CONV_THRESHOLD = 89
...
export async function gatherWeeklyFacts({ stGet, stPageAll, supabase, tenantId, weekEnd }) {
  ...
  // ST threshold conversions: a job that invoiced >= $89 (ST's line — an $89
  // consult fee converts; verified against ST's own Electrical number) in
  // the week converted, Sold estimate or not — trip-fee-only visits never do.
  const rev90Of = (invs) => {
    const byJob = new Map()
    for (const i of invs) { const jid = (i.job || {}).id; if (jid) byJob.set(jid, (byJob.get(jid) || 0) + (Number(i.subTotal) || 0)) }
    return new Set([...byJob.entries()].filter(([, amt]) => amt >= CONV_THRESHOLD).map(([jid]) => jid))
  }
  const rev90Week = rev90Of(invWeek)
  const rev90Prior = rev90Of(invPrior)

  // Installs are never opportunities (Brandyn, Sep 2026) — an install fulfills
  // an estimate sold on an earlier call. Drop install-typed jobs from every
  // presented/opportunity set before any close-rate or opps math.
  try {
    const ids = [...new Set([...presentedJobs, ...presentedPrior])]
    const batches = []
    for (let i = 0; i < ids.length; i += 50) {
      batches.push(safe(stGet(`/jpm/v2/tenant/${tenantId}/jobs?ids=${ids.slice(i, i + 50).join(',')}&pageSize=50`), null))
    }
    const results = await Promise.all(batches)
    const inst = new Set()
    for (const r of results) for (const j of (r?.data || [])) {
      if (/install/i.test((catById.get(String(j.jobTypeId)) || {}).name || '')) inst.add(j.id)
    }
    for (const set of [presentedJobs, presentedPrior,
      ...TRADES.map(t => byTrade[t].presented), ...TRADES.map(t => byTrade[t].soldJobs),
      ...Object.values(presentedPriorByTrade)]) {
      for (const j of inst) set.delete(j)
    }
  } catch (e) { console.warn('leadership install filter:', e.message) }
  ...
  // ── Close rate, ServiceTitan's way (the ONE definition, Sep 2026) ─────────
  // Same math as the department TVs: an opportunity is a job that RAN this
  // week on a sales-shaped job type (maintenance tune-ups auto-generate option
  // sheets and are not opportunities; unmapped types still count), never an
  // install, that had estimate activity OR invoiced >= $89; it converted on a
  // Sold estimate OR >= $89 invoiced. Departments attribute by the tech's
  // business unit, like the TV rosters. The old estimate-created-only
  // denominator read lower than ST on every trade.
  const SALES_OPP_CATS = new Set(['repair', 'other', 'free_estimate'])
  const inPrior = (iso) => { const t = Date.parse(iso || ''); return !Number.isNaN(t) && t >= Date.parse(pStartIso) && t < Date.parse(pEndIso) }
  const soldPriorJobs = new Set(sold6w.filter(e => inPrior(e.soldOn)).map(e => e.jobId).filter(Boolean))
  const weekAppts = appts.filter(a => inWeek(a.start) && a.status !== 'Canceled')
  const priorAppts = appts.filter(a => inPrior(a.start) && a.status !== 'Canceled')
  const techJobs = new Map()        // techId -> Set(jobId) this week
  const techJobsPrior = new Map()   // same, prior week (WoW row only)
  ...
  try { await assignInto(priorAppts, techJobsPrior) } catch (e) { console.warn('leadership prior assignments:', e.message) }
  ...
  const jobInstall = new Set() // install-typed jobs — never opportunities
  try {
    const ranIds = [...new Set([...techJobs.values(), ...techJobsPrior.values()].flatMap(s => [...s]))]
    for (let i = 0; i < ranIds.length; i += 50) {
      ...
      for (const j of (r?.data || [])) {
        ...
        if (!info) continue
        if (/install/i.test(info.name)) jobInstall.add(j.id)
  ...
  const oppShaped = (j) => { if (jobInstall.has(j)) return false; const c = jobCat.get(j); return c == null || SALES_OPP_CATS.has(c) }
  const stClose = (ranSet, presented, conv) => {
    let opps = 0, won = 0
    for (const j of ranSet) {
      if (!oppShaped(j)) continue
      if (!presented.has(j) && !conv.has(j)) continue
      opps++; if (conv.has(j)) won++
    }
    return { opps, conv: won, rate: opps ? won / opps : null }
  }
  const ranOf = (map, trade) => { const out = new Set(); for (const [tid, jobs] of map) { if (trade && techTrade(tid) !== trade) continue; for (const j of jobs) out.add(j) } return out }
  const convWeek = new Set([...soldJobIds, ...rev90Week])
  const convPrior = new Set([...soldPriorJobs, ...rev90Prior])
  const closeWeekAll = stClose(ranOf(techJobs), presentedJobs, convWeek)
  const closePriorAll = stClose(ranOf(techJobsPrior), presentedPrior, convPrior)
  const closeByTrade = Object.fromEntries(TRADES.map(t => [t, stClose(ranOf(techJobs, t), presentedJobs, convWeek)]))
  ...
  const scorecard = TRADES.map(t => {
    ...
    const soldJobs = closeByTrade[t].conv
  ...
  for (const [techId, jobs] of techJobs) {
    ...
    // Installs fulfill an already-sold estimate — excluded from opportunities
    // (Brandyn, Sep 2026), same rule as the Department TV boards.
    const oppJobs = [...jobs].filter(j => { if (jobInstall.has(j)) return false; const c = jobCat.get(j); return c == null || SALES_OPP_CATS.has(c) })
    const convOf = (j) => soldJobIds.has(j) || rev90Week.has(j)
    const presented = oppJobs.filter(j => presentedJobs.has(j) || convOf(j)).length
    const soldJobs = oppJobs.filter(convOf).length
