...
import { loadAdminAgents } from './adminAgents.js'
import { fetchSalesCloseInputs, computeSalesClose, rollupSalesClose, techStats } from './salesClose.js'
...
export const DEFAULT_BUDGETS = {
  // Company weekly sales goal = the department budgets added up (HVAC 210k +
  // Plumbing 110k + Electrical 80k + Garage 30k = $430k). The sheet this page
  // replaced carried a separate $350k KPI goal next to those same department
  // budgets, so the KPI card said $350k while the scorecard summed to $430k
  // (Brandyn, Sep 20). One number now: set weeklySalesGoal in
  // app_settings.leadership_budgets only to deliberately override the sum.
  weeklySalesGoal: null,
  monthlySalesTarget: 1516667,   // override per month on the page
  annualSalesTarget: 18200000,   // override on the page
  monthlyRevenueTarget: 1375920,
  dept: {
    // gm targets: Brandyn, Sep 20 2026 (true gross margin after burdened labor).
    // oppsPerDay: the $20M plan's daily sales-opportunity goals (Brandyn,
    // Sep 21 2026) — 12 + 14 + 9 + 5 = 40 a day company-wide, on effective
    // days (Mon–Fri = 1, Saturday = ½, Sunday = 0 → 5.5 a week).
    HVAC: { budget: 210000, conv: 0.7, gm: 0.48, labor: 0.25, rev: 195000, oppsPerDay: 12 },
    Plumbing: { budget: 110000, conv: 0.7, gm: 0.54, labor: 0.25, rev: 100000, oppsPerDay: 14 },
    Electrical: { budget: 80000, conv: 0.7, gm: 0.54, labor: 0.25, rev: 75000, oppsPerDay: 9 },
    'Garage Doors': { budget: 30000, conv: 0.7, gm: 0.56, labor: 0.25, rev: 25000, oppsPerDay: 5 },
  },
  // kpi.gm (company) is derived: the department targets weighted by their
  // revenue targets — set it here only to deliberately override that.
  kpi: { booking: 0.8, clubs: 15, reviews: 20, close: 0.7, gm: null },
}
...
// ── ServiceTitan job costing (Reporting API) ────────────────────────────────
// ST's per-job costing carries the real material / equipment / purchase-order
// costs. Its labor line is commission-only and tiny (wk of Sep 14: HVAC $7.8k
// of labor on $153k of revenue), so ST's own gross margin overstates. We keep
// ST's NON-labor costs and swap in the burdened labor model (ADP actuals when
// uploaded). The report is the tenant's saved "Job Costing" report; the
// Reporting API allows roughly one data call a minute, so a 429 is waited out.
const JOB_COSTING_REPORT_ID = 10774798
let _jcReportId = null
async function jobCostingRows({ stGet, stPost, tenantId, from, to }) {
  if (!stPost) return null
  const data = async (id, page) => {
    const body = { parameters: [{ name: 'DateType', value: 0 }, { name: 'From', value: from }, { name: 'To', value: to }, { name: 'IncludeAdjustmentInvoices', value: true }] }
    for (let attempt = 0; ; attempt++) {
      try { return await stPost(`/reporting/v2/tenant/${tenantId}/report-category/operations/reports/${id}/data?page=${page}&pageSize=500`, body) }
      catch (e) {
        const m = /Try again in (\d+) seconds/i.exec(e.message || '')
        if (!m || attempt >= 2) throw e
        await new Promise(r => setTimeout(r, Math.min(Number(m[1]) + 2, 70) * 1000))
      }
    }
  }
  if (!_jcReportId) {
    // The saved report id is stable for this tenant; fall back to a name lookup.
    let id = JOB_COSTING_REPORT_ID
    try { await stGet(`/reporting/v2/tenant/${tenantId}/report-category/operations/reports/${id}`) }
    catch {
      id = null
      for (let page = 1; page <= 5 && !id; page++) {
        const d = await stGet(`/reporting/v2/tenant/${tenantId}/report-category/operations/reports?pageSize=50&page=${page}`)
        id = (d?.data || []).find(r => r.name === 'Job Costing')?.id || null
        if (!d?.hasMore) break
      }
      if (!id) throw new Error('no "Job Costing" report in ServiceTitan')
    }
    _jcReportId = id
  }
  const rows = []
  for (let page = 1; page <= 10; page++) {
    const d = await data(_jcReportId, page)
    const fields = (d?.fields || []).map(f => f.name)
    for (const r of (d?.data || [])) rows.push(Object.fromEntries(fields.map((f, i) => [f, r[i]])))
    if (!d?.hasMore) break
  }
  return rows
}

export async function gatherWeeklyFacts({ stGet, stPageAll, stPost, supabase, tenantId, weekEnd, getFieldCoaching }) {
  ...
  let [estSold6w, estCreated, estCreatedPrior, invWeek, invPrior, invMtd, telecom, telecomPrior,
    ...
    safe(stPageAll(pg => `/jpm/v2/tenant/${tenantId}/appointments?startsOnOrAfter=${pStartIso}&pageSize=500&page=${pg}`, 12000), []),
    safe(supabase.from('job_type_spiffs').select('st_job_type_id, name, category').then(r => r.data || []), []),
  ])
  // Admins' calls are test calls and phone cover, not CSR performance —
  // out of booking %, the CSR table and the channel rates (Brandyn, Sep 14).
  const admins = await loadAdminAgents(supabase)
  telecom = telecom.filter(c => !admins.isAdminCall(c))
  telecomPrior = telecomPrior.filter(c => !admins.isAdminCall(c))
  ...
  if (!(Number(budgets.weeklySalesGoal) > 0)) {
    budgets.weeklySalesGoal = TRADES.reduce((a, t) => a + (Number((budgets.dept || {})[t]?.budget) || 0), 0)
  }
  if (!(Number(budgets.kpi?.gm) > 0)) {
    const wRev = TRADES.reduce((a, t) => a + (Number((budgets.dept || {})[t]?.rev) || 0), 0)
    const wGm = TRADES.reduce((a, t) => a + (Number((budgets.dept || {})[t]?.gm) || 0) * (Number((budgets.dept || {})[t]?.rev) || 0), 0)
    budgets.kpi = { ...(budgets.kpi || {}), gm: wRev ? wGm / wRev : 0.5 }
  }
  ...
  // ── Estimates presented this week (by trade), feeding the ST-style close below ──
  ...
  // ── Jobs ran per tech this week (appointments → assignments) ─────────────
  const weekAppts = appts.filter(a => inWeek(a.start) && a.status !== 'Canceled')
  const techJobs = new Map()        // techId -> Set(jobId) this week
  const assignInto = async (list, into) => {
    const ids = list.map(a => a.id).filter(Boolean)
    const apptById = new Map(list.map(a => [a.id, a]))
    for (let i = 0; i < ids.length; i += 50) {
      ...
      for (const asg of (r?.data || [])) {
        ...
        if (!into.has(asg.technicianId)) into.set(asg.technicianId, new Set())
        into.get(asg.technicianId).add(appt.jobId)
      }
    }
  }
  try { await assignInto(weekAppts, techJobs) } catch (e) { console.warn('leadership assignments:', e.message) }
  ...
  let buTrade = new Map()
  try {
    const [t, bus] = await Promise.all([
      stGet(`/settings/v2/tenant/${tenantId}/technicians?active=true&pageSize=500`),
      stGet(`/settings/v2/tenant/${tenantId}/business-units?pageSize=200`),
    ])
    buTrade = new Map((bus?.data || []).map(b => [String(b.id), tradeOf(b.name)]))
  ...
  const techTrade = (techId) => { const i = techNames.get(String(techId)) || {}; return tradeOf(i.bu) || buTrade.get(String(i.buId)) || null }

  const jobFlags = new Map()   // jobId -> { name, buId } — callback-shaped visits
  try {
    ...
    for (let i = 0; i < ranIds.length; i += 50) {
      ...
      for (const j of (r?.data || [])) {
        ...
        if (info && /callback|warranty|recall|concern/i.test(info.name)) jobFlags.set(j.id, { name: info.name, buId: j.businessUnitId })
      }
    }
  } catch (e) { console.warn('leadership job flags:', e.message) }

  // ── Close rate = ServiceTitan's Sales close (lib/salesClose.js) ──────────
  // The Technician Scorecard / BU dashboard number Brandyn compares against.
  // One pull spans last week through this week; each week is scored on its
  // own window. Departments attribute by the tech's business unit (roster
  // logic), falling back to the job's BU.
  const blankClose = () => ({ opps: 0, closed: 0, rate: null })
  let closeWeekAll = blankClose(), closePriorAll = blankClose(), closeByTrade = {}, closeResult = null
  let closePriorByTrade = {}, closeDailyAll = blankClose(), closeDailyByTrade = {}
  try {
    const closeInputs = await fetchSalesCloseInputs({ stGet, stPageAll, tenantId, startIso: pStartIso, endIso })
    const grouping = {
      groupOfTech: (tech, job) => techTrade(tech) || buTrade.get(String(job?.businessUnitId)) || null,
      groupOfJob: (job) => buTrade.get(String(job?.businessUnitId)) || null,
    }
    closeResult = computeSalesClose(closeInputs, { startIso, endIso })
    const priorResult = computeSalesClose(closeInputs, { startIso: pStartIso, endIso: pEndIso })
    closeWeekAll = rollupSalesClose(closeResult).all || blankClose()
    closePriorAll = rollupSalesClose(priorResult).all || blankClose()
    closeByTrade = rollupSalesClose(closeResult, grouping)
    closePriorByTrade = rollupSalesClose(priorResult, grouping)
    // Daily-average basis: an in-progress week counts only COMPLETED days —
    // today's half-finished opps would otherwise sit on a denominator that
    // hasn't earned them yet.
    const dailyEndIso = partial ? denverDayBounds(asOf).startIso : endIso
    const dailyResult = partial ? computeSalesClose(closeInputs, { startIso, endIso: dailyEndIso }) : closeResult
    closeDailyAll = rollupSalesClose(dailyResult).all || blankClose()
    closeDailyByTrade = rollupSalesClose(dailyResult, grouping)
  } catch (e) { console.warn('leadership sales close:', e.message) }
  for (const t of TRADES) { closeByTrade[t] = closeByTrade[t] || blankClose(); closePriorByTrade[t] = closePriorByTrade[t] || blankClose(); closeDailyByTrade[t] = closeDailyByTrade[t] || blankClose() }

  // ── Department scorecard ──────────────────────────────────────────────────
  const scorecard = TRADES.map(t => {
    const d = byTrade[t]
    const cfg = budgets.dept[t] || {}
    const opps = closeByTrade[t].opps
    const soldJobs = closeByTrade[t].closed
    const close = closeByTrade[t].rate
  ...
  const closeAll = closeWeekAll.rate
  const closePrior = closePriorAll.rate

  const outboundOf = (calls) => calls.reduce((a, c) => a + (((c.leadCall || c).direction || '') === 'Outbound' ? 1 : 0), 0)
  // ── Opportunities per day vs the $20M plan ────────────────────────────────
  // Sales opportunities (ServiceTitan's rule, lib/salesClose.js) ÷ effective
  // days. Goals are per department in budgets.dept[t].oppsPerDay; company =
  // the sum. Paired with close rate: volume and conversion are the two levers.
  const effDaysBetween = (a, b) => {
    let n = 0
    for (let d = a; d <= b; d = shiftDate(d, 1)) { const wd = new Date(`${d}T12:00:00Z`).getUTCDay(); n += wd === 0 ? 0 : wd === 6 ? 0.5 : 1 }
    return n
  }
  const dailyThrough = partial ? shiftDate(asOf, -1) : weekEnd
  const effDays = dailyThrough >= weekStart ? effDaysBetween(weekStart, dailyThrough) : 0
  const effDaysPrior = effDaysBetween(pWeekStart, pWeekEnd)
  const r1 = (n) => Math.round(n * 10) / 10
  const oppRow = (trade, opps, priorOpps, goal, closeRate, convTarget) => {
    const perDay = effDays ? opps / effDays : null
    const priorPerDay = effDaysPrior ? priorOpps / effDaysPrior : null
    const gap = perDay != null && goal ? perDay - goal : null
    const onPlan = gap != null && gap >= 0
    const closeOk = closeRate == null || !convTarget || closeRate >= convTarget
    let verdict = '—'
    if (perDay != null && goal) {
      const more = `${r1(-gap)} more a day (≈ ${Math.round(-gap * 5.5)} a week)`
      const closeTxt = `close rate ${Math.round((closeRate || 0) * 100)}% vs ${Math.round(convTarget * 100)}%`
      verdict = onPlan && closeOk ? '✓ on plan'
        : onPlan ? `volume is there — closing is the fix (${closeTxt})`
        : closeOk ? `book ${more}`
        : `book ${more} AND close better (${closeTxt})`
    }
    return { trade, opps, perDay: perDay != null ? r1(perDay) : null, goal, gapPerDay: gap != null ? r1(gap) : null,
      weeklyGap: gap != null ? Math.round(gap * 5.5) : null, priorOpps, priorPerDay: priorPerDay != null ? r1(priorPerDay) : null,
      closeRate, convTarget, onPlan: perDay != null && goal ? onPlan : null, verdict }
  }
  const oppsDaily = {
    effDays, effDaysPrior, through: dailyThrough, weekStart, partial,
    rows: TRADES.map(t => oppRow(t, closeDailyByTrade[t].opps, closePriorByTrade[t].opps, Number(budgets.dept[t]?.oppsPerDay) || 0, closeByTrade[t].rate, budgets.dept[t]?.conv || 0.7)),
  }
  oppsDaily.all = oppRow('AWESOME', closeDailyAll.opps, closePriorAll.opps, TRADES.reduce((a, t) => a + (Number(budgets.dept[t]?.oppsPerDay) || 0), 0), closeWeekAll.rate, budgets.kpi.close || 0.7)

  const kpis = [
    { kpi: 'Booking %', thisWk: bk.rate, lastWk: bkP.rate, goal: budgets.kpi.booking, fmt: 'pct' },
    { kpi: 'Outbounds Made', thisWk: outboundOf(telecom), lastWk: outboundOf(telecomPrior), goal: null, fmt: 'int' },
  ...
  // Per-tech close = the tech's own line on ST's Technician Scorecard
  // (same result as the department numbers above).
  let technicians = []
  for (const [techId, jobs] of techJobs) {
    ...
    const st = closeResult ? techStats(closeResult, techId) : { opps: 0, closed: 0, rate: null }
    const presented = st.opps
    const soldJobs = st.closed
  ...
  let csrs = []
  let qaWkAvg = null, qaWkPrior = null
  try {
    ...
    try {
      ...
      evals = evals.filter(e => !admins.isAdminName(e.rep))   // admins aren't QA'd
      const scored = evals.filter(e => e.pct != null)
      if (scored.length) qaWkAvg = Math.round(scored.reduce((a, e) => a + Number(e.pct), 0) / scored.length)
      // Prior week, same recordings-join, for the WoW row.
      try {
        const { startIso: pS } = denverDayBounds(pWeekStart)
        const { endIso: pE } = denverDayBounds(pWeekEnd)
        const { data: pRecs } = await supabase.from('call_recordings').select('call_sid')
          .eq('direction', 'inbound').gte('call_started_at', pS).lt('call_started_at', pE).limit(3000)
        const pSids = [...new Set((pRecs || []).map(r => r.call_sid).filter(Boolean))]
        let pEvals = []
        for (let i = 0; i < pSids.length; i += 200) {
          const { data } = await supabase.from('call_evaluations').select('id, rep, pct').in('call_sid', pSids.slice(i, i + 200))
          if (data?.length) pEvals = pEvals.concat(data)
        }
        const pScored = pEvals.filter(e => e.pct != null && !admins.isAdminName(e.rep))
        if (pScored.length) qaWkPrior = Math.round(pScored.reduce((a, e) => a + Number(e.pct), 0) / pScored.length)
      } catch {}
  ...
  let spendWkTotal = 0, spendPriorWkTotal = 0, mtdSpend = 0
  ...
  try {
    ...
    // Partial week: charge only the days that have happened, or a 2-day-old
    // week eats 7 days of spend against 2 days of revenue and everything
    // reads 0x.
    const wkDays = Math.max(1, Math.min(7, Math.floor((Date.parse(asOf) - Date.parse(weekStart)) / 86400000) + 1))
    for (const r of costRows) {
      const n = campName.get(r.campaignId)
      if (n) spendByName.set(n, (spendByName.get(n) || 0) + (Number(r.dailyCost) || 0) * wkDays)
    }
    for (const c of telecom) {
      const lc = c.leadCall || c
      if ((lc.direction || '') !== 'Inbound') continue
    ...
    // Week totals for the KPI table: this week's spend, prior week's spend
    // (its own month's rates), and month-to-date spend through asOf.
    spendWkTotal = costRows.reduce((a, r) => a + (Number(r.dailyCost) || 0) * wkDays, 0)
    const pMid = new Date(`${pWeekStart}T12:00:00Z`)
    let pCostRows = costRows
    if (pMid.getUTCMonth() !== wkMid.getUTCMonth()) {
      pCostRows = await safe(stPageAll(pg => `/marketing/v2/tenant/${tenantId}/costs?year=${pMid.getUTCFullYear()}&month=${pMid.getUTCMonth() + 1}&pageSize=200&page=${pg}`, 2000), [])
    }
    spendPriorWkTotal = pCostRows.reduce((a, r) => a + (Number(r.dailyCost) || 0) * 7, 0)
    const daysElapsed = Number(String(asOf).slice(8, 10)) || 0
    mtdSpend = costRows.reduce((a, r) => a + (Number(r.dailyCost) || 0) * daysElapsed, 0)
  ...
  // ── Gross margin: ST job costing costs + true labor, per department ───────
  // gm = (invoiced revenue − ST non-labor job costs − burdened field labor)
  // ÷ revenue, all on THIS week's invoices. Labor is the commission model on
  // this week's commissionable revenue, or the uploaded ADP actual when one
  // exists for this week-ending. gmSt is ST's own job-costing margin (its
  // labor is commission-only), kept for reference.
  let gmAll = null, gmStAll = null, gmPriorAll = null, gmSource = 'model'
  const gmStByTrade = {}
  try {
    const jcRollup = (rows) => {
      const out = {}
      for (const t of TRADES) out[t] = { revenue: 0, nonLabor: 0, stLabor: 0, materials: 0, po: 0, stGm: 0, jobs: 0 }
      for (const r of (rows || [])) {
        const t = tradeOf(r.JobBusinessUnit); if (!t) continue
        const rev = Number(r.TotalRevenue) || 0, cost = Number(r.TotalCosts) || 0, lab = Number(r.TotalLaborCosts) || 0
        const o = out[t]
        o.revenue += rev; o.nonLabor += Math.max(0, cost - lab); o.stLabor += lab
        o.materials += (Number(r.MaterialTotals) || 0) + (Number(r.EquipmentCosts) || 0)
        o.po += Number(r.PurchaseOrderCosts) || 0
        o.stGm += Number(r.GrossMargin) || 0; o.jobs++
      }
      return out
    }
    const adpAll = await settingsJson(supabase, 'adp_payroll_actuals', {})
    const adpWk = adpAll[weekEnd]
    if (adpWk?.totals?.cost) gmSource = 'adp'
    const modelLabor = (commBase) => commBase * burden.commissionRate * burden.poolUplift * burden.fieldBurden
    // Both weeks of the KPI row use the SAME labor basis — ADP actuals only
    // when THIS week has them, else the model for both. Mixing (this week
    // model, last week ADP) printed a +21pt "improvement" that was really
    // the labor source changing (Sep 21).
    const adpPrior = adpAll[pWeekEnd]
    const laborFor = (trade, commBase, adpRow) => (gmSource === 'adp' && adpRow?.cost) ? adpRow.cost : modelLabor(commBase)
    const wk = jcRollup(await jobCostingRows({ stGet, stPost, tenantId, from: weekStart, to: asOf }))
    let revAll = 0, costAll = 0, stGmSum = 0
    for (const row of scorecard) {
      const o = wk[row.trade]
      const laborWk = laborFor(row.trade, byTrade[row.trade].commBase, adpWk?.byTrade?.[row.trade])
      const gm = o.revenue ? (o.revenue - o.nonLabor - laborWk) / o.revenue : null
      row.gm = gm
      row.gmSt = o.revenue ? o.stGm / o.revenue : null
      row.gmCosts = { revenue: r0(o.revenue), materials: r0(o.materials), po: r0(o.po), otherNonLabor: r0(o.nonLabor - o.materials - o.po), labor: r0(laborWk), stLabor: r0(o.stLabor), jobs: o.jobs, laborSource: (gmSource === 'adp' && adpWk?.byTrade?.[row.trade]?.cost) ? 'adp' : 'model' }
      gmStByTrade[row.trade] = row.gmSt
      revAll += o.revenue; costAll += o.nonLabor + laborWk; stGmSum += o.stGm
    }
    gmAll = revAll ? (revAll - costAll) / revAll : null
    gmStAll = revAll ? stGmSum / revAll : null
    // Prior week for the KPI row — its labor is the scorecard's LW true labor
    // (ADP actual when uploaded). Best effort: the report API is rate-limited.
    try {
      const pw = jcRollup(await jobCostingRows({ stGet, stPost, tenantId, from: pWeekStart, to: pWeekEnd }))
      let pRev = 0, pCost = 0
      for (const row of scorecard) { const o = pw[row.trade]; pRev += o.revenue; pCost += o.nonLabor + laborFor(row.trade, lwCommByTrade[row.trade] || 0, adpPrior?.byTrade?.[row.trade]) }
      gmPriorAll = pRev ? (pRev - pCost) / pRev : null
    } catch (e) { console.warn('leadership gross margin (prior week):', e.message) }
    const at = kpis.findIndex(k => k.kpi === 'Close Rate')
    kpis.splice(at >= 0 ? at + 1 : kpis.length, 0, { kpi: 'Gross Margin', thisWk: gmAll, lastWk: gmPriorAll, goal: budgets.kpi.gm, fmt: 'pct' })
  } catch (e) { console.warn('leadership gross margin:', e.message) }
  ...
  // Late KPI rows (need QA, spend, and MTD numbers computed above/below):
  // QA after Outbounds, CPL after Lead Volume, Spend%-of-revenue after CPL.
  kpis.splice(kpis.findIndex(k => k.kpi === 'Outbounds Made') + 1, 0,
    { kpi: 'Overall QA Score', thisWk: qaWkAvg != null ? qaWkAvg / 100 : null, lastWk: qaWkPrior != null ? qaWkPrior / 100 : null, goal: 0.9, fmt: 'pct' })
  kpis.splice(kpis.findIndex(k => k.kpi === 'Lead Volume') + 1, 0,
    { kpi: 'Overall CPL', thisWk: bk.total && spendWkTotal ? r0(spendWkTotal / bk.total) : null, lastWk: bkP.total && spendPriorWkTotal ? r0(spendPriorWkTotal / bkP.total) : null, goal: null, fmt: 'money', lowerIsBetter: true },
    { kpi: 'Mkt Spend % of Revenue (MTD)', thisWk: mtdRevenue ? mtdSpend / mtdRevenue : null, lastWk: null, goal: 0.12, fmt: 'pct', lowerIsBetter: true })
  ...
  // ── Coaching levers: where coaching moves the most money ─────────────────
  // Every coachable gap sized in monthly dollars with the scorecard's own
  // math (gap × volume × value, ×4.33 weeks), plus the behavior evidence —
  // Field Pro scorecard steps for techs, QA criteria ↔ booking for CSRs — so
  // the agenda can say WHAT to coach and who first, ranked by upside.
  let coaching = null
  try {
    coaching = await buildCoachingLevers({ supabase, admins, scorecard, technicians, csrs, telecom, telecomPrior, budgets, bk, salesTotal, asOf, getFieldCoaching })
  } catch (e) { console.warn('leadership coaching levers:', e.message) }
  ...
  return {
    ...
    totals: {
      ...
      gm: gmAll, gmSt: gmStAll, gmTarget: budgets.kpi.gm || null, gmSource, gmStByTrade,
      unpaid: r0(unpaidTotal),
      opps: closeWeekAll.opps, closeRate: closeAll,
...
// ── Coaching levers ─────────────────────────────────────────────────────────
// Monthly upside = weekly gap × 4.33. Associations (step ↔ won, criterion ↔
// booked) are correlations from small samples — the agenda must cite n and
// call them that. Nothing here is a forecast; it's "what closing the gap is
// worth at this week's volume and ticket".
const WEEKS_PER_MONTH = 4.33
const normName = (n) => String(n || '').toLowerCase().replace(/[^a-z0-9]/g, '')
async function buildCoachingLevers({ supabase, admins, scorecard, technicians, csrs, telecom, telecomPrior, budgets, bk, salesTotal, asOf, getFieldCoaching }) {
  const m = (v) => Math.round((v || 0) * WEEKS_PER_MONTH)
  const levers = []
  // Field: close rate and ticket per department.
  for (const row of scorecard) {
    if (!row.opps || row.closeRate == null || !row.avgSale) continue
    const gap = row.convTarget ? Math.max(0, row.convTarget - row.closeRate) : 0
    levers.push({
      dept: row.trade, lever: 'close rate', current: Math.round(row.closeRate * 100) / 100, target: row.convTarget,
      weeklyOpps: row.opps, avgSale: row.avgSale,
      monthlyUpsideToTarget: m(gap * row.opps * row.avgSale),
      monthlyUpsidePer5Pts: m(0.05 * row.opps * row.avgSale),
    })
    levers.push({ dept: row.trade, lever: 'average ticket', current: row.avgSale, monthlyUpsidePer10Pct: m(0.10 * row.sales) })
  }
  // Call center: booking rate. A booked lead call is worth this week's sales
  // per booked call (captures the field's conversion too).
  const salesPerBooked = bk?.booked ? salesTotal / bk.booked : null
  const bookGoal = budgets?.kpi?.booking || 0.8
  if (bk?.total && salesPerBooked) {
    levers.push({
      dept: 'Call Center', lever: 'booking rate', current: Math.round(bk.rate * 100) / 100, target: bookGoal,
      weeklyLeadCalls: bk.total, salesPerBookedCall: Math.round(salesPerBooked),
      monthlyUpsideToTarget: m(Math.max(0, bookGoal - bk.rate) * bk.total * salesPerBooked),
      monthlyUpsidePer5Pts: m(0.05 * bk.total * salesPerBooked),
    })
  }
  levers.sort((a, b) => (b.monthlyUpsideToTarget ?? b.monthlyUpsidePer10Pct ?? 0) - (a.monthlyUpsideToTarget ?? a.monthlyUpsidePer10Pct ?? 0))

  // Field Pro evidence (Siro scorecards), when the server provides it.
  let field = null
  if (typeof getFieldCoaching === 'function') {
    try { field = await getFieldCoaching(String(asOf).slice(0, 7)) } catch (e) { console.warn('coaching field pro:', e.message) }
  }
  // Techs with the most money in their close-rate gap this week, with their
  // own lowest Field Pro steps.
  const convOf = Object.fromEntries(scorecard.map(r => [r.trade, r.convTarget]))
  const avgOf = Object.fromEntries(scorecard.map(r => [r.trade, r.avgSale]))
  const techGaps = technicians.filter(t => t.presented >= 3 && t.trade && convOf[t.trade] && t.closeRate != null)
    .map(t => {
      const ticket = t.avgTicket || avgOf[t.trade] || 0
      const fp = field?.techs?.[normName(t.name)] || null
      return {
        name: t.name, trade: t.trade, opps: t.presented, closeRate: Math.round(t.closeRate * 100) / 100, target: convOf[t.trade], avgTicket: ticket,
        monthlyUpsideToTarget: m(Math.max(0, convOf[t.trade] - t.closeRate) * t.presented * ticket),
        fieldProScore: fp?.score ?? null, fieldProCalls: fp?.scoredCalls ?? 0, lowestSteps: fp?.lowestSteps || [],
      }
    })
    .filter(t => t.monthlyUpsideToTarget > 0).sort((a, b) => b.monthlyUpsideToTarget - a.monthlyUpsideToTarget).slice(0, 8)

  // CSRs: QA criteria vs whether the call booked (ServiceTitan callType),
  // over this week + last week, joined on the eval's ST call id.
  let callCenter = null
  try {
    const callType = new Map()
    for (const c of [...(telecom || []), ...(telecomPrior || [])]) {
      const lc = c.leadCall || c
      if ((lc.direction || '') === 'Inbound' && ['Booked', 'Unbooked'].includes(lc.callType)) callType.set(`st-${lc.id}`, lc.callType)
    }
    const sids = [...callType.keys()]
    let evals = []
    for (let i = 0; i < sids.length; i += 200) {
      const { data } = await supabase.from('call_evaluations').select('call_sid, rep, scores').in('call_sid', sids.slice(i, i + 200))
      if (data?.length) evals = evals.concat(data)
    }
    evals = evals.filter(e => !admins?.isAdminName?.(e.rep))
    const crit = new Map(), repCrit = new Map()
    for (const e of evals) {
      const booked = callType.get(e.call_sid) === 'Booked'
      const repKey = String(e.rep || '').trim().toLowerCase()
      for (const it of ((e.scores || {}).items || [])) {
        if (!it.applicable || !it.criterion) continue
        const met = (Number(it.earned) || 0) >= (Number(it.max) || 0)
        const c = crit.get(it.criterion) || { criterion: it.criterion, section: it.section || null, metN: 0, metBooked: 0, missN: 0, missBooked: 0 }
        if (met) { c.metN++; if (booked) c.metBooked++ } else { c.missN++; if (booked) c.missBooked++ }
        crit.set(it.criterion, c)
        if (!met && repKey) {
          if (!repCrit.has(repKey)) repCrit.set(repKey, new Map())
          const rc = repCrit.get(repKey); rc.set(it.criterion, (rc.get(it.criterion) || 0) + 1)
        }
      }
    }
    const weeks = 2
    const criteria = [...crit.values()].map(c => {
      const bookedWhenMet = c.metN ? c.metBooked / c.metN : null
      const bookedWhenMissed = c.missN ? c.missBooked / c.missN : null
      const lift = bookedWhenMet != null && bookedWhenMissed != null ? bookedWhenMet - bookedWhenMissed : null
      // If the calls that missed it had booked at the "met" rate:
      const extraPerWeek = lift != null && lift > 0 ? (c.missN / weeks) * lift : 0
      return {
        criterion: c.criterion, section: c.section, missRate: Math.round(c.missN / Math.max(1, c.metN + c.missN) * 100) / 100,
        callsMet: c.metN, callsMissed: c.missN,
        bookedWhenMet: bookedWhenMet == null ? null : Math.round(bookedWhenMet * 100) / 100,
        bookedWhenMissed: bookedWhenMissed == null ? null : Math.round(bookedWhenMissed * 100) / 100,
        monthlyUpsideIfFixed: salesPerBooked ? m(extraPerWeek * salesPerBooked) : null,
      }
    })
    const solid = criteria.filter(c => c.callsMet >= 10 && c.callsMissed >= 10)
    const csrGaps = (csrs || []).filter(c => c.leadCalls >= 10 && c.bookRate != null)
      .map(c => {
        const rc = repCrit.get(String(c.name).trim().toLowerCase())
        const most = rc ? [...rc.entries()].sort((a, b) => b[1] - a[1]).slice(0, 2).map(([k, n]) => ({ criterion: k, missedOn: n })) : []
        return { name: c.name, leadCalls: c.leadCalls, bookRate: Math.round(c.bookRate * 100) / 100, qa: c.qa,
          monthlyUpsideToGoal: salesPerBooked ? m(Math.max(0, bookGoal - c.bookRate) * c.leadCalls * salesPerBooked) : null, mostMissed: most }
      })
      .filter(c => c.monthlyUpsideToGoal > 0).sort((a, b) => b.monthlyUpsideToGoal - a.monthlyUpsideToGoal).slice(0, 5)
    callCenter = {
      evaluatedLeadCalls: evals.length, weeksCovered: weeks, bookingGoal: bookGoal,
      criteriaLinkedToBooking: solid.filter(c => c.monthlyUpsideIfFixed > 0).sort((a, b) => b.monthlyUpsideIfFixed - a.monthlyUpsideIfFixed).slice(0, 5),
      mostMissedCriteria: [...criteria].filter(c => c.callsMet + c.callsMissed >= 10).sort((a, b) => b.missRate - a.missRate).slice(0, 5),
      csrGaps,
    }
  } catch (e) { console.warn('coaching call center:', e.message) }

  return {
    method: 'monthlyUpside = weekly gap × volume × value × 4.33. Close rate: (target − actual) × opps × avg sale. Ticket: 10% of the week\'s sales. Booking: (goal − actual) × lead calls × sales per booked call. Step/criterion links are correlations with sample sizes, not proof.',
    levers,
    techGaps,
    fieldPro: field ? { month: field.month, byTrade: field.byTrade, notRecording: (field.notRecording || []).slice(0, 12) } : null,
    callCenter,
  }
}

// ── AI: exec summary, action items, coaching callouts ───────────────────────
// The model's tool call comes back malformed now and then — a JSON array
// encoded as a STRING where the schema says array (the page then printed
// `["Booking % hit…", "Clubs sold…"]` as one bullet), a list glued into one
// string with newlines or bullets, or an output cut off at max_tokens with
// no tool_use block at all (a blank section). Parse defensively and retry
// the call before giving up.
const aiList = (v) => {
  if (Array.isArray(v)) return v.flatMap(x => typeof x === 'string' && /^\s*\[/.test(x) ? aiList(x) : [x])
  if (v == null || v === '') return []
  if (typeof v !== 'string') return [String(v)]
  const t = v.trim()
  if (/^\[/.test(t)) { try { const p = JSON.parse(t); if (Array.isArray(p)) return aiList(p) } catch {} }
  const lines = t.split(/\r?\n/).map(l => l.replace(/^\s*(?:[-•*✓☐]|\d+[.)])\s*/, '').trim()).filter(Boolean)
  return lines.length > 1 ? lines : [t]
}

export async function generateAgendaAI(facts, anthropicKey, { attempts = 3 } = {}) {
  if (!anthropicKey) return null
  for (let attempt = 1; attempt <= attempts; attempt++) {
    const out = await agendaAiOnce(facts, anthropicKey)
    if (out && (out.headline || out.wins.length || out.challenges.length)) return out
    console.warn(`leadership AI: empty result (attempt ${attempt} of ${attempts})`)
    if (attempt < attempts) await new Promise(r => setTimeout(r, 4000 * attempt))
  }
  return null
}

async function agendaAiOnce(facts, anthropicKey) {
  const ctl = new AbortController()
  const timer = setTimeout(() => ctl.abort(), 150_000)   // 4k tokens incl. coaching focus
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: `You are the operations analyst for Awesome Home Services (HVAC, plumbing, electrical, garage doors — Colorado Springs), writing the AI section of the owner's WEEKLY leadership meeting agenda. You get the week's numbers as JSON: department scorecard vs budgets, KPIs with week-over-week, technician and CSR leaderboards, marketing channels with weekly spend/CPL/ROAS (revenue = this week's invoices attributed to the channel's jobs; flag channels where spend is high and ROAS is under 1x, and channels with high junkPct), oppsDaily (sales opportunities per effective day this week vs the $20M plan's daily goal per department — HVAC 12, Plumbing 14, Electrical 9, Garage 5, company 40 — with last week's per-day for comparison and a verdict pairing volume with close rate), true burdened labor, and callbacks.

\`mtd\` and \`ytd\` carry monthly and yearly sales/revenue pacing (paced target = budget prorated to today); cite pace when a department or the company is meaningfully ahead or behind. Scorecard rows carry callbacks — callback/warranty jobs run in that department this week — and gm: THIS week's true gross margin (invoiced revenue minus ServiceTitan job-costing material/equipment/PO costs minus burdened field labor; gmCosts breaks it down, laborSource says model or adp) against gmTarget; gmSt is ServiceTitan's own job-costing margin, which counts only commission labor and always reads higher — never quote gmSt as the margin. ALL labor/trueLabor figures cover the PRIOR week (labor.weekEnd), because payroll and invoice settlement lag — never present them as this week's cost. If labor.source is "adp", trueLabor figures are ACTUAL payroll from the uploaded ADP invoice, not a model — treat labor %-of-revenue findings as hard numbers then.
...
Your reader is an operations manager scanning this in 60 seconds before the meeting. Write SHORT. Wins and challenges are one line each, number first, no dependent clauses; a win must be a real win (goal hit, meaningful improvement), never a silver-lining spin on a miss. Action items are imperative sentences a manager can assign as-is ("Coach X on Y — closed 2 of 9"), max ~15 words each, grouped under the department they belong to. Use oppsDaily (per-day opportunities vs goal) together with closeRate to distinguish lead-flow problems from conversion problems: under goal = booking/lead flow; at goal but under 70% close = conversion. Only cite numbers present in the data — never invent. If a number looks like a data gap, skip it rather than guessing. No fluff, no filler, no restating the tables.

COACHING FOCUS (given separately as "Coaching levers"): say where coaching buys the biggest increase in sales, revenue or a KPI. \`levers\` sizes each department's gap in MONTHLY dollars (close rate to target, +5 pts close, +10% ticket, call-center booking rate to goal); \`techGaps\` are the techs with the most money in their close-rate gap, with their lowest Field Pro steps; \`fieldPro.byTrade\` has each trade's Field Pro (Siro in-home call scorecard, 0-100) score, lowest steps, and stepWinLift (win rate when a step scored 50+ vs under 50, with call counts); \`callCenter\` has QA criteria linked to booking (booked rate when met vs missed, with call counts and the monthly upside if the misses booked at the met rate), the most-missed criteria, and CSRs with the most money in their booking gap. Rank by monthly upside. For each department with a real lever, name ONE behavior to coach (a Field Pro step for techs, a QA criterion for CSRs), who to coach first, the dollar upside from the lever math, the evidence, and one concrete drill. Pick the behavior with evidence of moving the number (stepWinLift / criteriaLinkedToBooking with decent counts) over the merely lowest-scoring one; when samples are small (under ~20 calls) say so and fall back to the lowest-scoring step. Never claim causation — say "calls that did X booked Y% vs Z%". Each department's action items should LEAD with its coaching action when it has one.`,
        tools: [{
          name: 'submit_agenda',
          ...
          input_schema: {
            ...
            properties: {
              ...
              wins: { type: 'array', items: { type: 'string' }, description: '2-4 one-line WINS, number first — what genuinely went right this week.' },
              challenges: { type: 'array', items: { type: 'string' }, description: '2-4 one-line CHALLENGES, number first — the misses and risks that deserve the meeting\'s attention.' },
              coachingFocus: {
                type: 'array',
                description: 'Where coaching moves sales/revenue/KPIs the most, ranked by monthly $ upside (max 6). One entry per department with a real lever.',
                items: {
                  type: 'object',
                  properties: {
                    dept: { type: 'string', description: 'HVAC, Plumbing, Electrical, Garage Doors, Call Center, or Company' },
                    focus: { type: 'string', description: 'The one behavior to coach, named as the scorecard step / QA criterion, max ~8 words.' },
                    upside: { type: 'string', description: 'Estimated monthly $ upside and which KPI moves, e.g. "~$42k/mo — close rate 60% → 70%".' },
                    evidence: { type: 'string', description: 'One line with the numbers: score/miss rate and the won/booked link with counts.' },
                    who: { type: 'string', description: 'Who to coach first (names from techGaps / csrGaps), or "whole team".' },
                    drill: { type: 'string', description: 'One concrete drill or ride-along exercise for this week.' },
                  },
                  required: ['dept', 'focus', 'upside', 'evidence', 'who', 'drill'],
                },
              },
              actionsByDept: {
                type: 'array',
            ...
            required: ['headline', 'wins', 'challenges', 'coachingFocus', 'actionsByDept'],
          },
        }],
        tool_choice: { type: 'tool', name: 'submit_agenda' },
        // Coaching levers go separately so the 26k slice of the facts can't cut them off.
        messages: [{ role: 'user', content: `This week's numbers:\n${JSON.stringify({ ...facts, coaching: undefined }).slice(0, 26000)}${facts.coaching ? `\n\nCoaching levers:\n${JSON.stringify(facts.coaching).slice(0, 12000)}` : ''}` }],
      }),
    })
    if (!r.ok) { console.warn('leadership AI: HTTP', r.status, (await r.text().catch(() => '')).slice(0, 200)); return null }
    const body = await r.json()
    const out = body?.content?.find(c => c.type === 'tool_use')?.input
    if (!out) { console.warn('leadership AI: no tool_use in response, stop_reason', body?.stop_reason); return null }
    const arr = (v) => aiList(v).map(x => (x && typeof x === 'object') ? x : String(x))
    // The model occasionally leaks its own tool-call markup into string
    // values ('<parameter name="1">…'). Strip anything tag-shaped — no AI
    // text here legitimately contains angle-bracket markup.
    const cleanAi = (v) => String(v).replace(/<[^>]{0,80}>/g, '').replace(/\s+/g, ' ').trim()
    return {
      headline: cleanAi(out.headline || ''),
      wins: arr(out.wins).slice(0, 4).map(cleanAi).filter(Boolean),
      challenges: arr(out.challenges).slice(0, 4).map(cleanAi).filter(Boolean),
      highlights: arr(out.highlights).slice(0, 5).map(cleanAi).filter(Boolean),
      coachingFocus: arr(out.coachingFocus).slice(0, 6)
        .filter(d => d && typeof d === 'object' && d.focus)
        .map(d => ({ dept: cleanAi(d.dept || ''), focus: cleanAi(d.focus || ''), upside: cleanAi(d.upside || ''), evidence: cleanAi(d.evidence || ''), who: cleanAi(d.who || ''), drill: cleanAi(d.drill || '') })),
      actionsByDept: arr(out.actionsByDept).slice(0, 6)
        .filter(d => d && typeof d === 'object')
        .map(d => ({ dept: cleanAi(d.dept || ''), actions: arr(d.actions).slice(0, 3).map(cleanAi).filter(Boolean) })),
    }
  } catch (e) {
...
export function renderLeadershipHtml(facts, ai, notes = {}) {
  ...
  const scoreRows = f.scorecard.map(d => [
    ...
    d.gm != null ? `${goalSpan(d.gm, d.gmTarget || 0.55, pct(d.gm))}${d.gmTarget ? ` <span style="color:${C.mute};font-size:11px">/ ${pct(d.gmTarget)}</span>` : ''}` : '—',
  ])
  ...
  scoreRows.push([
    ...
    totals.gm != null ? `<b>${goalSpan(totals.gm, totals.gmTarget || 0.55, pct(totals.gm))}</b>` : '—',
  ])
  const gmNote = totals.gm != null
    ? `GM = invoiced revenue − ServiceTitan job-costing material / equipment / PO costs − burdened field labor (${totals.gmSource === 'adp' ? 'actual ADP payroll for this week' : `${Math.round((f.labor.factors.commissionRate || 0) * 100)}% commission × ${f.labor.factors.poolUplift} pool × ${f.labor.factors.fieldBurden} burden on this week's commissionable revenue`}). ST's own job costing counts only commission labor and reads ${['HVAC', 'Plumbing', 'Electrical', 'Garage Doors'].map(t => `${t} ${totals.gmStByTrade?.[t] != null ? pct(totals.gmStByTrade[t]) : '—'}`).join(' · ')}.`
    : 'GM unavailable this run (ServiceTitan job-costing report did not answer).'
  ...
  const kpiRows = f.kpis.map(k => {
    ...
    const goalHit = k.goal != null && k.thisWk != null && (k.lowerIsBetter ? k.thisWk <= k.goal : k.thisWk >= k.goal)
    const deltaGood = delta != null && (k.lowerIsBetter ? delta <= 0 : delta >= 0)
    const thisCell = k.goal == null ? fmtV(k.thisWk)
      : `<span style="color:${goalHit ? C.good : C.bad};font-weight:700">${fmtV(k.thisWk)}</span>`
    return [
      esc(k.kpi), thisCell, fmtV(k.lastWk),
      delta == null ? '—' : `<span style="color:${deltaGood ? C.good : C.bad};font-weight:700">${delta >= 0 ? '+' : ''}${k.fmt === 'pct' ? `${Math.round(delta * 100)}pt` : k.fmt === 'money' ? money(delta) : Math.round(delta)}</span>`,
      k.goal == null ? '—' : `${k.lowerIsBetter ? '≤' : ''}${fmtV(k.goal)} ${k.thisWk != null ? (goalHit ? '✓' : '✗') : ''}`,
    ]
  })

  const od = f.oppsDaily || { rows: [], all: null, effDays: 0 }
  const oppRow = (r, bold) => {
    const w = (s) => bold ? `<b>${s}</b>` : s
    const tone = r.onPlan == null ? C.mute : r.onPlan ? C.good : C.bad
    return [
      w(esc(r.trade)),
      w(String(r.goal || '—')),
      r.perDay != null ? `<span style="color:${tone};font-weight:700">${r.perDay}</span>` : '—',
      r.priorPerDay != null ? String(r.priorPerDay) : '—',
      r.gapPerDay == null ? '—' : r.gapPerDay >= 0 ? `<span style="color:${C.good}">on goal</span>` : `<span style="color:${C.bad}">${Math.abs(r.gapPerDay)} short</span>`,
    ]
  }
  const oppRows = [...od.rows.map(r => oppRow(r, false)), ...(od.all ? [oppRow(od.all, true)] : [])]
  ...
  // Technician / CSR leaderboards were removed from the agenda (Brandyn, Sep 20, 2026).
  ...
  const aiCoach = Array.isArray(ai?.coachingFocus) ? ai.coachingFocus.filter(d => d && d.focus) : []
  const aiHtml = ai ? `
    <div style="background:#F0F6FB;border-left:4px solid ${C.accent};border-radius:8px;padding:14px 16px">
      ...
      ${aiWins.length ? `<div style="font-size:11px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:${C.good};margin:0 0 6px">Wins</div>
        ${aiWins.map(s => `<div style="font-size:13px;color:${C.ink};line-height:1.55;margin-bottom:4px">✓ ${esc(s)}</div>`).join('')}` : ''}
      ${aiChallenges.length ? `<div style="font-size:11px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:${C.bad};margin:10px 0 6px">Challenges</div>
        ${aiChallenges.map(s => `<div style="font-size:13px;color:${C.ink};line-height:1.55;margin-bottom:4px">• ${esc(s)}</div>`).join('')}` : ''}
      ...
      ${aiCoach.length ? `<div style="font-size:11px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:${C.accent};margin:12px 0 6px">Coaching focus — biggest upside first</div>
        ${aiCoach.map(d => `
          <div style="background:#fff;border:1px solid ${C.line};border-radius:8px;padding:9px 12px;margin-bottom:7px">
            <div style="font-size:13px;color:${C.ink}"><b>${esc(d.dept)}:</b> ${esc(d.focus)} <span style="color:${C.good};font-weight:700">· ${esc(d.upside)}</span></div>
            ${d.evidence ? `<div style="font-size:12px;color:${C.mute};line-height:1.5;margin-top:2px">${esc(d.evidence)}</div>` : ''}
            ${d.who ? `<div style="font-size:12.5px;color:${C.ink};line-height:1.5;margin-top:3px"><b>Coach first:</b> ${esc(d.who)}</div>` : ''}
            ${d.drill ? `<div style="font-size:12.5px;color:${C.ink};line-height:1.5"><b>Drill:</b> ${esc(d.drill)}</div>` : ''}
          </div>`).join('')}` : ''}
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:860px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    ${section('Department scorecard', table(
      ['Dept', 'Wk Sales', 'Budget', 'Var', 'Wk Rev', 'Rev Tgt', 'Close / Tgt', 'Avg Sale', 'Opps', 'Missed $', '5★', 'Clubs', 'Callbacks', 'GM / Tgt'], scoreRows) + `<div style="font-size:11px;color:${C.mute};margin-top:6px">${gmNote}</div>`)}

    ${section('Company KPIs (WoW)', table(['KPI', 'This Wk', 'Last Wk', 'Δ', 'Goal'], kpiRows))}

    ${section('Opportunities per day vs the $20M plan', `
      <div style="font-size:11.5px;color:${C.mute};margin-bottom:8px">Average sales opportunities run per working day (Mon–Fri full, Saturday half), this week and last, against the plan's daily goal.${od.partial ? ` This week counts ${od.effDays} completed days through ${od.through}.` : ''}</div>
      ${table(['Dept', 'Goal / day', 'This week / day', 'Last week / day', 'Short of goal / day'], oppRows)}`)}
    ...
    <div style="margin-top:26px;padding-top:12px;border-top:1px solid ${C.line};font-size:11px;color:${C.mute};line-height:1.6">
      Sales = estimates marked Sold · Revenue = invoiced subtotals · Close rate = ServiceTitan's Sales close (the Technician Scorecard number): opportunities are completed, chargeable sales jobs credited to the tech on the first appointment; closed when the job's sold estimates first reach the $90 sold threshold, credited to the seller ·
