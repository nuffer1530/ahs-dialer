...
export function renderLeadershipHtml(facts, ai, notes = {}) {
  ...
  const oppRow = (r, bold) => {
    ...
    return [
      w(esc(r.trade)), w(String(r.opps)),
      r.perDay != null ? `<span style="color:${tone};font-weight:700">${r.perDay}</span>` : '—',
      w(String(r.goal || '—')),
      r.gapPerDay != null ? signed(r.gapPerDay, (n) => String(Math.round(Math.abs(n) * 10) / 10)) : '—',
      r.priorPerDay != null ? String(r.priorPerDay) : '—',
      `<span style="color:${tone}">${esc(r.verdict)}</span>`,
    ]
  }
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:860px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    ${section('Opportunities per day vs the $20M plan', `
      <div style="font-size:11.5px;color:${C.mute};margin-bottom:8px">Sales opportunities (ServiceTitan's rule) ÷ effective days — Mon–Fri = 1, Saturday = ½, Sunday = 0, so a full week is 5.5${od.partial ? ` (this week: ${od.effDays} days through ${od.through})` : ''}. Goals are the $20M plan: HVAC 12 · Plumbing 14 · Electrical 9 · Garage 5 = 40 a day, paired with the 70% close target.</div>
      ${table(['Dept', 'Opps', 'Per day', 'Goal / day', 'Gap / day', 'Last wk / day', 'Verdict'], oppRows)}`)}
