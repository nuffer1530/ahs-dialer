...
export async function gatherWeeklyFacts({ stGet, stPageAll, supabase, tenantId, weekEnd }) {
  ...
  const [estYoy, invYoy, estPm, invPm, estYtd] = await Promise.all([
    ...
    safe(stPageAll(pg => `/sales/v2/tenant/${tenantId}/estimates?soldAfter=${yearIso}&soldBefore=${endIso}&pageSize=500&page=${pg}`, 30000), []),
  ])
  ...
  const scorecard = TRADES.map(t => {
    ...
    return {
      ...
      // GP after true burdened field labor. ST item costs are all $0 in this
      // tenant, so materials can't be included — label it that way, always.
      gp: r0(d.revenue - trueLabor),
      gpPct: d.revenue ? (d.revenue - trueLabor) / d.revenue : null,
  ...
  // Callbacks per department: map each flagged job's business unit to a trade.
  try {
    if (jobFlags.size) {
      ...
      for (const { buId } of jobFlags.values()) {
        const row = scorecard.find(r => r.trade === buTrade.get(String(buId)))
        if (row) row.callbacks++
      }
    }
  } catch (e) { console.warn('leadership callback depts:', e.message) }
  ...
  let ytdSales = 0
  const ytdSalesByTrade = {}
  for (const e of estYtd) {
    if ((e.status || {}).name !== 'Sold') continue
    const amt = Number(e.subtotal) || 0
    ytdSales += amt
    const t = tradeOf(e.businessUnitName)
    if (t) ytdSalesByTrade[t] = (ytdSalesByTrade[t] || 0) + amt
  }
...
export function renderLeadershipHtml(facts, ai, notes = {}) {
  ...
  const scoreRows = f.scorecard.map(d => [
    ...
    d.gpPct != null ? `<span style="color:${d.gpPct >= (d.gmTarget || 0.55) ? C.good : C.bad}">${money(d.gp)} (${pct(d.gpPct)})</span>` : '—',
  ])
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:860px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    ${(() => {
      ...
      return `<table style="width:100%;border-collapse:separate;border-spacing:8px 0;margin-top:8px"><tr>
        ${card('MTD Sales', money(f.mtd.sales), `budget ${money(mSales)} · pace ${money(mSales * mFrac)} · proj ${money(f.mtd.salesProjected)}`)}
        ${card('MTD Revenue', money(f.mtd.revenue), `budget ${money(mRev)} · pace ${money(mRev * mFrac)} · proj ${money(f.mtd.projected)}`)}
        ${f.ytd ? card('YTD Sales', money(f.ytd.sales), `target ${money(ySales)} · pace ${money(ySales * yFrac)} · proj ${money(f.ytd.projected)}`) : ''}
      </tr></table>`
    })()}
    ...
    ${section('Department scorecard', table(
      ['Dept', 'Wk Sales', 'Budget', 'Var', 'Wk Rev', 'Rev Tgt', 'Close / Tgt', 'Avg Sale', 'Opps', 'Missed $', 'Callbacks', 'GP after labor'], scoreRows))}
    <div style="font-size:11px;color:${C.mute};margin-top:4px">GP after labor = revenue − true burdened field labor (ADP model); material costs aren't tracked in ST, so this is not full gross margin.</div>
    ...
    ${topCsrs.length ? section('CSRs — top 3 + lowest', table(['CSR', 'Score', 'Booked', 'Lead calls', 'Book rate', 'QA'],
