...
export async function gatherWeeklyFacts({ stGet, stPageAll, supabase, tenantId, weekEnd }) {
  ...
  // ── CSRs: answered, booked, QA → composite ────────────────────────────────
  let csrs = []
  try {
    const [{ data: tasks }, { data: bookings }, { data: evals }] = await Promise.all([
      supabase.from('call_tasks').select('agent_name, state, answered_at, abandoned').gte('queued_at', startIso).lt('queued_at', endIso),
      supabase.from('andi_bookings').select('csr_name').gte('booked_at', startIso).lt('booked_at', endIso),
      supabase.from('call_evaluations').select('rep, pct').gte('created_at', startIso).lt('created_at', endIso),
    ])
    const byRep = new Map()
    const get = (n) => {
      if (!byRep.has(n)) byRep.set(n, { name: n, answered: 0, booked: 0, qa: [] })
      return byRep.get(n)
    }
    for (const t of (tasks || [])) if (t.state === 'answered' || t.answered_at) get(t.agent_name || 'Unknown').answered++
    for (const b of (bookings || [])) get(b.csr_name || 'Unknown').booked++
    for (const e of (evals || [])) if (e.pct != null && e.rep) get(e.rep).qa.push(Number(e.pct))
    csrs = [...byRep.values()].map(r => ({
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: `You are the operations analyst for Awesome Home Services (HVAC, plumbing, electrical, garage doors — Colorado Springs), preparing the owner's WEEKLY leadership meeting agenda. You get the week's numbers as JSON: department scorecard vs budgets, KPIs with week-over-week, technician and CSR leaderboards (composite = revenue 40% / close rate 30% / $-per-opportunity 30%), marketing channels with WoW movement, opportunities ran vs needed (oppsNeeded = what it would have taken to hit budget at that close rate and ticket), true burdened labor (labor.laborPctOfRevenue uses the real ADP burden, not ST's wage-blind number), and callbacks.

Rules: only cite numbers present in the data — never invent. Action items must be specific enough to assign in the meeting: name the department, person, or channel, the number that justifies it, and the concrete move. Prefer the non-obvious: a department that hit sales but missed on opportunity volume vs one that had the opps and couldn't close (oppsNeeded vs opps tells you which); a channel spending calls without booking; a tech whose callbacks undercut their sales; labor % vs the 25% target. If a number looks like a data gap, say so instead of drawing a false conclusion. No fluff, no congratulations padding.`,
        tools: [{
          name: 'submit_agenda',
          ...
          input_schema: {
            type: 'object',
            properties: {
              headline: { type: 'string', description: 'One sentence: the single most important thing about the week.' },
              summary: { type: 'array', items: { type: 'string' }, description: '3-5 bullets: the week in numbers, each with its figure.' },
              actionItems: {
                type: 'array',
                items: {
                  type: 'object',
                  properties: {
                    action: { type: 'string', description: 'The concrete move, with the number that justifies it.' },
                    owner: { type: 'string', description: 'Suggested owner: a department, role, or named person from the data.' },
                  },
                  required: ['action'],
                },
              },
              techNotes: { type: 'array', items: { type: 'string' }, description: 'Per-technician callouts worth saying out loud — top performers with the why, and coaching cases (behavior, not person).' },
              csrNotes: { type: 'array', items: { type: 'string' }, description: 'Per-CSR callouts, same rules.' },
              watchouts: { type: 'array', items: { type: 'string' }, description: 'Risks forming in the numbers — trends about to become problems.' },
            },
    ...
    return {
      ...
      summary: (out.summary || []).slice(0, 6),
      actionItems: (out.actionItems || []).slice(0, 8),
      techNotes: (out.techNotes || []).slice(0, 6),
      csrNotes: (out.csrNotes || []).slice(0, 6),
      watchouts: (out.watchouts || []).slice(0, 5),
    }
  } catch (e) {
...
export function renderLeadershipHtml(facts, ai, notes = {}) {
  ...
  const aiHtml = ai ? `
    <div style="background:#F0F6FB;border-left:4px solid ${C.accent};border-radius:8px;padding:14px 16px">
      ...
      ${(ai.summary || []).map(s => `<div style="font-size:13px;color:${C.ink};line-height:1.55;margin-bottom:5px">• ${esc(s)}</div>`).join('')}
      ${(ai.actionItems || []).length ? `<div style="font-size:11px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:${C.mute};margin:12px 0 6px">Action items</div>
      ...
      ${(ai.watchouts || []).length ? `<div style="font-size:11px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:${C.mute};margin:12px 0 6px">Watch-outs</div>
        ${ai.watchouts.map(w => `<div style="font-size:13px;color:${C.warn};line-height:1.5;margin-bottom:4px">⚠ ${esc(w)}</div>`).join('')}` : ''}
    </div>` : ''
