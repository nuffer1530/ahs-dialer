...
const DEFAULT_BUDGETS = {
  ...
  dept: {
    ...
    HVAC: { budget: 210000, conv: 0.7, gm: 0.48, labor: 0.25, rev: 195000 },
    Plumbing: { budget: 110000, conv: 0.7, gm: 0.54, labor: 0.25, rev: 100000 },
    Electrical: { budget: 80000, conv: 0.7, gm: 0.54, labor: 0.25, rev: 75000 },
    'Garage Doors': { budget: 30000, conv: 0.7, gm: 0.56, labor: 0.25, rev: 25000 },
  },
...
export async function gatherWeeklyFacts({ stGet, stPageAll, stPost, supabase, tenantId, weekEnd }) {
  ...
  for (const t of TRADES) closeByTrade[t] = closeByTrade[t] || blankClose()
  ...
  try {
    ...
    for (const row of scorecard) {
      ...
      const laborWk = adpWk?.byTrade?.[row.trade]?.cost || modelLabor(byTrade[row.trade].commBase)
      const gm = o.revenue ? (o.revenue - o.nonLabor - laborWk) / o.revenue : null
      row.gm = gm
      row.gmSt = o.revenue ? o.stGm / o.revenue : null
      row.gmCosts = { revenue: r0(o.revenue), materials: r0(o.materials), po: r0(o.po), otherNonLabor: r0(o.nonLabor - o.materials - o.po), labor: r0(laborWk), stLabor: r0(o.stLabor), jobs: o.jobs, laborSource: adpWk?.byTrade?.[row.trade]?.cost ? 'adp' : 'model' }
    ...
    try {
      ...
      let pRev = 0, pCost = 0
      for (const row of scorecard) { const o = pw[row.trade]; pRev += o.revenue; pCost += o.nonLabor + (row.trueLabor || 0) }
  ...
  const oppRows = f.scorecard.map(d => {
    ...
    if (d.oppsNeeded != null) {
      if (d.opps >= d.oppsNeeded) verdict = d.sales >= d.budget
        ...
        // Empty slots come first — staffing isn't the fix while trucks sat unfilled.
        const fillable = Math.max(0, Math.min(d.oppsNeeded, cap) - d.opps)
        if (fillable > 0) verdict = `<span style="color:${C.bad}">fix: book ${fillable} more — trucks had room</span>`
          + (d.oppsNeeded > cap ? `<br><span style="color:${C.mute};font-size:11px">and even full, trucks max at ${cap} of ${d.oppsNeeded} needed at this close rate${atTarget != null && atTarget <= cap ? ` — at ${Math.round(d.convTarget * 100)}% target close, ~${atTarget} calls hits budget` : ''}</span>` : '')
        else verdict = `<span style="color:${C.bad}">fix: capacity — trucks ran full at ${cap}; needs ${d.oppsNeeded} at this close rate</span>`
