...
export async function gatherWeeklyFacts({ stGet, stPageAll, supabase, tenantId, weekEnd }) {
  ...
  const scorecard = TRADES.map(t => {
    ...
    const soldJobs = [...d.presented].filter(j => d.soldJobs.has(j) || rev90Week.has(j)).length
  ...
  const closeAll = presentedJobs.size ? [...presentedJobs].filter(j => soldJobIds.has(j) || rev90Week.has(j)).length / presentedJobs.size : null
  ...
  const closePrior = presentedPrior.size ? [...presentedPrior].filter(j => soldPriorJobs.has(j) || rev90Prior.has(j)).length / presentedPrior.size : null
