...
export function renderLeadershipHtml(facts, ai, notes = {}) {
  ...
  return `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:860px;margin:0 auto;color:${C.ink};padding:8px">
    ...
    ${f.marketing.length ? section('Marketing channels — spend vs return (spend prorated to days elapsed)', table(['Channel', 'Spend', 'Leads', 'Booked', 'Rate', 'Jobs', 'Revenue', 'CPL', 'ROAS'],
      f.marketing.map(m => [esc(m.name) + (m.spend > 0 && m.junkPct > 0.5 && (m.calls + m.junk) >= 5 ? ` <span style="color:${C.warn};font-size:10px;font-weight:700">&#9888; junk</span>` : ''),
