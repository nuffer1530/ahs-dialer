...
export async function gatherWeeklyFacts({ stGet, stPageAll, supabase, tenantId, weekEnd }) {
  ...
  // ST threshold conversions (Brandyn, Sep 11): a job that invoiced >= $90 in
  ...
  const rev90Of = (invs) => {
    ...
    return new Set([...byJob.entries()].filter(([, amt]) => amt >= 90).map(([jid]) => jid))
  }
  ...
  // install, that had estimate activity OR invoiced >= $90; it converted on a
  // Sold estimate OR >= $90 invoiced. Departments attribute by the tech's
