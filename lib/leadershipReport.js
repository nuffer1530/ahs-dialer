...
export async function gatherWeeklyFacts({ stGet, stPageAll, supabase, tenantId, weekEnd }) {
  ...
  const dayOfYear = Math.round((Date.parse(`${weekEnd}T12:00:00Z`) - Date.parse(`${yearStart}T12:00:00Z`)) / 86400000) + 1
  const daysInYear = Number(weekEnd.slice(0, 4)) % 4 === 0 ? 366 : 365
  const ytd = {
    sales: r0(ytdSales),
