...
export async function loadAdminAgents(supabase) {
  // ServiceTitan's shared company login is a system account, not a CSR.
  const empty = { profileIds: new Set(), stUserIds: new Set(), names: new Set(['awesome account', 'awesome home services']) }
  try {
    const [{ data: profs }, { data: maps }] = await Promise.all([
      // Operations managers dial customers too, but they're field-side
      // leadership, not CSRs — same exclusion as admins.
      supabase.from('profiles').select('id, name, email').in('role', ['admin', 'ops_manager', 'call_center_manager']),
