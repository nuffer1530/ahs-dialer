...
export function aggregateAdpActuals(parsed, deptMap, rates = REGISTER_BURDEN_DEFAULTS, managerOverrides = []) {
  ...
  const unmatched = []
  // Ops managers count as overhead, not their trade's field labor — a trade's
  // labor % should grade the crew, not the manager's own salary (Cedric,
  // Sep 2026). Same dollars, different bucket: the all-in total is unchanged.
  const mgrSet = new Set((managerOverrides || []).map(n => String(n).toLowerCase().replace(/\s+/g, ' ').trim()))
  for (const e of parsed.employees) {
    const m = employees[e.name.toLowerCase()] || null
    const benWk = m ? (Number(m.ben_month) || 0) * 12 / 52 : 0
    const isMgr = mgrSet.has(String(e.name).toLowerCase().replace(/\s+/g, ' ').trim())
    // Register rows know their department; invoice rows need the mapping.
    const dept = isMgr ? 'Ops Management' : (e.dept || m?.dept || '(unmapped)')
    const trade = isMgr ? 'Office/Overhead' : (tradeOf(dept) || 'Office/Overhead')
    const isField = isMgr ? false : (e.wc != null && e.wc !== '' ? e.wc.startsWith('5537') : trade !== 'Office/Overhead')
