...
- **`src/pages/DialerLayout.jsx`** — the app shell's logic: rep status tracking (writes `status_events` rows on status change), role gates and all authenticated routes. The chrome itself (redesign stage 1, Sep 2026) is `src/components/shell/`: `nav.js` groups the routes into hubs (Phones · Dispatch · Calls · Team · Leadership) with the same role gates, `Sidebar` (dark rail, TV-boards launcher, avatar menu = My Page + Settings + theme), `TopBar` (hub title + tabs), `PhoneDock` (status + back-to-call, bottom-right beside Ask Andi), `CommandPalette` (⌘K) and `MobileTabBar`. A new page = a route in DialerLayout plus a tab in a hub in `nav.js`. Everyone lands on `/home`: `HomePage` renders the business Home (`/api/home`) for admins and ops managers, and `components/home/MyDayHome` (`/api/home/me`, plus `/api/dispatch/center` for dispatchers) for CSRs and dispatchers.
